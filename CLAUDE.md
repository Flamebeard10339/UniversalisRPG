...
- `npm run authorbot -- <brief> [--minutes <n>]` — hand one brief to a coding agent over a copy of `content/` and count what it reached for. With `--minutes` the run is told when a minute is left and cut off two minutes past the clock; no tool is ever refused for time. The engine is refused unless `--open`, and every reach for it is a question the oracle did not answer, which is what the run is for. It writes nothing in this checkout. **This is how a module gets authored**; the `authoring` skill is the procedure, and a hand-rolled subagent throws the measurement away
...
- `npm run rename-section -- <kind>:<old id> <new id>` — writes one section under a new id everywhere the world reads it: its own heading, every reference to it — bare inside its module, qualified from another, and the flags, nodes and actions minted beneath it — and every key and value inside a `# save`. Which lines hold a reference is read off each kind's own walk rather than off a list of sites, so a word standing in prose or in a comment is left as it was written. Unlike the two above it writes nothing under `src/` or `scripts/`: a section id is a word, and one there is likelier a fixture that happens to spell it, so every tracked file outside the world that names the section is reported instead, to be read by hand. Write the old id whole, as `location:tulsa.forge`, where two modules declare one of a kind; `--at <dir>` points it at another world and `--dry-run` writes nothing. Nothing is written unless the world loads afterwards and its registry and its namespace differ by exactly this one id
