...
5. **Testing procedure**
  ...
  3. CI: `.github/workflows/test.yml` runs `npx tsc --noEmit`, `npm test` and `npm run layer-check` on push and PR, plus `npm run audit-status` and `npm run tasks -- doctor` on the ubuntu leg. `doctor` fails on one condition only: a `docs/tasks.jsonl` line that will not parse
