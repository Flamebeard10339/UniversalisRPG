...
- `npm run balance -- <save> [<action-spec>]` — what every offer the engine puts in front of a player standing on that save pays over one hour of game time. Every figure is read off a run rather than reckoned: it writes a `# test` per offer, walks it under several seeds until the world's clock reaches the far end of the window, and prints cycles, how much of the window the offer itself ran for, xp and items an hour, and the engine's own sentence wherever a run stopped short. The window is the only denominator, and a run whose offer stops early — a room emptied, the player killed — spends what is left of it standing where the world put them, so dying at seven seconds costs the rest of the hour. A second loose word narrows the sweep; `--at`, `--seeds`, `--window` and `--all` narrow or widen it further, and `--help` prints the rest. Reach for this before changing any stat, drop or rate: it is an hour of world per offer per seed, so it is called when there is a balance question and is never a gate
