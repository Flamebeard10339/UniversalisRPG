...
const world = loadUniverse(withEngineLocale([...fixtureSources(), floor(DIGS_TO_TWO)]));

describe('the floors sheet', () => {
  it('reads a route\'s goal off its own closing assert, qualified the way the world names the skill', () => {
    expect(goalOf(world, 'town-floor.dig-to-2')).toEqual({ skill: 'core.digging', level: 2 });
  });
  ...
  it('walks a floor and stands its minutes beside what the curve asks for the level it reached', () => {
    ...
    expect(lines[0]).toContain(minutesToReachOn(ladderForSkill(world, 'core.digging')!, 2).toFixed(1));
  });
