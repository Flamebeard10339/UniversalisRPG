...
const WORKER_IDS = ['worker/comment-rule', 'worker/mutation-proof', 'worker/record-decisions', 'worker/aim-at-the-clause', 'worker/label-from-data', 'worker/absence-is-not-evidence', 'worker/file-findings'];
...
const PLANNER_IDS = ['planner/state-the-invariant', 'planner/guard-placement', 'planner/who-else-computes', 'planner/proof-shape-follows-design', 'planner/name-delegated-decisions'];
...
describe('every instruction in the four briefs has an id', () => {
  ...
  it('resolves each of the shipped ids to a live lesson', () => {
...
describe('a citation is matched exactly as given, and every one is accounted for', () => {
  ...
  it('never answers with a lesson whose id is not the citation', () => {
    ...
    expect(liveIds.filter((id) => findLesson(id) !== undefined)).toHaveLength(23);
  });
...
describe('two lessons cannot share one id', () => {
  ...
  it('the shipped ids are distinct', () => {
    expect(new Set(allLessons().map((lesson) => lesson.id)).size).toBe(23);
  });
