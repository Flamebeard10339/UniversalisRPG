...
- base: `ba96a5bfb249c05299a52350be81fe51c65760db`
- head: `fffda451530e2550ba739059bbefdaea49917275`
- proof 1: met — Re-measured at this head rather than carried forward from pass 1. Pass 1's own manifest, unmodified, re-run against the repaired tree: `npm run mutate -- C:\Users\yonat\AppData\Local\Temp\audit-runtime-has-an-order-because-it-has-no-cycles-pass1-mutations.json` reports 8 killed, 0 survived, 0 unstable, 0 errored. c1-detector-loses-the-back-edge is killed by 5 tests in scripts/lib/acyclic.test.ts and c1-component-of-two-is-not-a-cycle by 2, both inside that file. The two subject-derivation mutations that pass 1 recorded as killed only after escalating to the whole suite now die inside scripts/lib/acyclic.test.ts itself, against the two cases added at c014b83: "the graph that answer was read off > holds the modules this repository ships" and "> holds the imports between them, resolved to modules". The file named as c1's proof now proves c1 without leaning on printedWords.test.ts and layers.test.ts. The Kahn re-derivation from pass 1 still stands and is re-runnable at C:\Users\yonat\AppData\Local\Temp\audit-runtime-has-an-order-because-it-has-no-cycles-pass1-kahn.txt.
- proof 2: met — The three mutations that pass 1 recorded as SURVIVED against the whole 3764-test suite are killed at this head, measured with pass 1's own manifest unmodified so the two passes are comparing the same mutants:
  c2-gate-reports-instead-of-failing    KILLED  2 failed of 30  [scripts/lib/layers.test.ts]
  c2-gate-names-no-closing-import       KILLED  2 failed of 30  [scripts/lib/layers.test.ts]
  c2-gate-names-no-module-on-the-cycle  KILLED  2 failed of 30  [scripts/lib/layers.test.ts]
Each names the same two tests, added at c014b83 and re-run at their own file with the mutation still applied: "layerCheckOutput > fails the run on a cycle, and names every module on it as well as the imports that close it" and "runLayerCheck > carries a cycle all the way to the exit code, naming the modules on it and the import that closes it". The first passes a hand-written `cycles` fixture and asserts the literal output lines, so the fixture is input and the expectation is output and the two do not move together; members and closing edge are asserted separately, so deleting either loop fails while the other still passes. The second drives a synthetic two-module cyclic tree end to end through runLayerCheck's effects to the exit code, so the gate is proved on a tree that has a cycle rather than on this checkout being clean. c2's proof target was also corrected at 8e21f9e from scripts/lib/acyclic.test.ts to scripts/lib/layers.test.ts: the gate lives in layers.ts and acyclic.test.ts owns the detector, and since audit-prompt builds the mutation manifest from proof targets, the old target would have aimed pass 3 at the wrong file again.
  Recorded by the same actor that commissioned the repair. That is not an independent look, and it is the whole of what this pass adds over pass 1: pass 1 graded c2 unmet on three survivors, and this pass re-runs those three exact mutants and records that they die. Nothing else about c2 was re-examined.
- proof 3: met — Unchanged at this head and re-measured. `src/runtime/state.ts` declares `GameState` and imports six modules — `../grammar/section`, `../grammar/tagClause`, `./error`, `./localized`, `./rng`, `./said` — none of which imports it back at any depth, which is the Kahn result from pass 1 restricted to this file and still empty at this head. The repair commits touched state.ts only to receive the `BuffInstance` comment that came home with its declaration at a60af5e; no import moved. c3-c4-cycles-never-reach-the-report is KILLED at scripts/lib/layers.test.ts, so the composition that would carry a back-edge into the report is still watched.
- proof 4: met — Unchanged at this head. `src/runtime` holds 49 shipped modules, all 49 their own unit, stratification depth 21 derived from its own imports and declared in no file, against a merge base of 42 modules resolving to 15 units. Re-run: `npm run inspect -- - < C:\Users\yonat\AppData\Local\Temp\audit-runtime-has-an-order-because-it-has-no-cycles-pass1-kahn.txt` and the -base.txt beside it. The repair added no manifest, no per-file configuration and no OUTSIDE_STACK entry.
- proof 5: met — Re-measured at this head rather than carried forward, because the repair touched src/content/registry.ts and src/content/module.ts. Loading all three shipped `content/*.dsl` through src/content/load.ts and canonicalising the whole load result gives registryBytes 330102, registrySha 058f85c2ab7db087e262b9722689b6aebdee2ac6fd358e712ea81334dbb94ab0 — byte-for-byte the value measured at the merge base ba96a5b through that commit's src/content/registry.ts in a worktree at pass 1. Serialising every module back out through roundTripUniverse gives printedBytes 51659, printedSha 327df9cc699b2b945bad6c288046bb4599ed11328d0332c6f08bc142bb6af430, differences [], diagnostics 0 — again identical to the base. Re-run: `npm run inspect -- - < C:\Users\yonat\AppData\Local\Temp\audit-runtime-has-an-order-because-it-has-no-cycles-pass1-registry-head.txt`. The repair's two content edits were both comments. `npm test` is green at 3768.
- proof 6: deferred — `npm run tasks -- merge-ready` still prints "NOT merge-ready", so this is checked-and-fails rather than met. Every leg that measures the code is green — tsc, npm test 3768, layer-check, audit-status, doctor, bytes, base — and this spec's own two legs are now green as well: `spec runtime-has-an-order-because-it-has-no-cycles ok pass — every member this branch declared is closed`. What is left red belongs to a different spec. Registering `every-pass-over-a-section-kind-derives-its-subjects` in this branch's store diff attached that spec's standing to this branch, so merge-ready now reports an open member and no recorded pass for a spec whose work has not started. That is the recorded friction `a-planning-branch-cannot-be-merge-ready-because-attaching-a-`, not a defect in what this branch built.
The goal — give `src/runtime` a reading order, so that a change to one of its modules can be understood from that module and the interfaces beneath it — holds without this clause and is discharged by c1 through c4, which are met on re-run evidence. c6 is a process gate over the branch's paperwork, and the one thing still holding it red is another spec's paperwork rather than anything about this branch's code. Deferring keeps that on the record as a tracked undelivered item instead of dropping it; the author has ruled that this branch merges on its recorded standing.
