...
- [c10] Neither generated artifact is written over work an auditor has already done, and neither is
  ...
  state. Both are keyed to the pass, so keeping a file never means handing pass N+1 the manifest
  pass N had already aimed under a step that tells it to aim one, and the notes saying which fields
  are the auditor's print whether the file was just written or kept.
...
- base: `4314b3e7691cf732471862872188f5254725c01f`
- head: `2d28759a61e7f7f49ae7381bd98c770772efd708`
- proof 1: met — Two clause-targeted mutations, both KILLED at narrow scope with no escalation (1 failed of 93, scope column naming the test alone), in a 28-entry hand-aimed run: delete scripts/tasks/audit.ts:209, the `moved:` branch of unresolvedTarget, and "a target whose title lives in another file is told where it actually is" fails; delete audit.ts:199, the found-in-named-file return that stops the escalation, and "a target that resolves in the file it names never pays for a wider search" fails on its search counter. Re-run: build a manifest naming those two lines and `npm run mutate` it.
- proof 2: met — Delete audit.ts:209 and the `nowhere` and `moved` messages collapse into one, killing "a title that exists nowhere is reported differently from one that merely moved" at narrow scope; rewrite audit.ts:198 so a missing file returns `nowhere` instead of `no-such-file` and it kills "a target naming a file absent from the checkout is reported as a missing file". Both narrow, no escalation, in the same 28-entry run. `resolveTarget`'s union still carries five states, the fifth being `unsearchable`, which is a superset of the clause.
- proof 3: met — All four targets KILLED at narrow scope, no escalation, in the 28-entry run: replace audit.ts:398 `const file = resolution.state === 'moved' ? resolution.foundIn[0] : resolution.file;` with `const file = resolution.file;` and "a manifest entry runs the test its clause names, in the file that test lives in" fails on the moved entry; replace `file: UNAIMED_FILE` at audit.ts:404 with `file: resolution.file` and "the manifest offers no guess at which line a clause is about" fails; replace `find: UNRETARGETED` on the same line with a literal find string and "a manifest entry nobody has aimed is refused by mutate rather than run green" fails; delete audit.ts:395, the omitted.push, and "an unresolved target is named as omitted rather than emitted into the manifest" fails.
Measured live, not read: `audit-prompt audit-brief-arrives-complete` wrote 26 entries to %TEMP%\mutations-audit-brief-arrives-complete.json, every one carrying `file` and `find` as sentinels and no `note` key at all; `npm run mutate` on it unedited was refused by name for all 26 before any baseline ran, and left no journal behind. `breakableAddedLines` and `candidateNote` are gone from the tree with no remaining reference.
What the clause does not promise, and is filed as a finding: the manifest path is not keyed to the pass, so pass N+1 inherits pass N's aimed file and the notes that would explain the sentinels are suppressed exactly then.
- proof 4: met — Delete audit.ts:643, the commit loop, and "the brief names the commits in its diff range and what each touched" fails at narrow scope, no escalation. The live brief for this spec carries all twenty-two commits in range 4314b3e..2d28759 with the files each touched, and the fourteen-file diff stat.
- proof 5: met — Delete audit.ts:143, the per-path ownership line, and "the brief answers ownership and prior art for every path in its diff" fails at narrow scope. The live brief answers ownership for all fourteen diff paths — the seven docs/, CLAUDE.md and .planning/ ones as declared unowned — and prints prior art over the seven owned paths: three [concept] lines, seven open collisions, five unreviewed, and "67 closed claim(s) not listed" as a count.
- proof 6: met — Delete audit.ts:635, the `toolLines(packageScripts())` loop, and "the brief names each tool an auditor may reach for, with the command that runs it" fails at narrow scope. The staleness check is derived rather than asserted: `toolLines` with `probe` removed emits `package.json has no "probe" script; this entry is stale`, `toolLines(null)` marks nothing, and all six named scripts are present so the live brief marks none.
- proof 7: met — Delete audit.ts:482, the branch-mismatch warning's `lines.push` argument, and "a slug whose spec this branch does not own is reported rather than ranged silently against HEAD" fails at narrow scope. Live: `npm run tasks -- audit-prompt tasks-roadmap` on this branch prints both warnings — the branch is working audit-brief-arrives-complete, and tasks-roadmap's last pass at 71c5aed is already an ancestor of base 4314b3e — and now suppresses both artifacts. `audit-prompt audit-brief-arrives-complete` prints neither and emits both. The pass-4 finding this clause carried, that steps 4 and 7 were ungated, is closed by c10.
- proof 8: met — All four targets KILLED at narrow scope, no escalation, in the 28-entry run: replace audit.ts:603 with `const argsKept = false;` and "writes the pass file the auditor fills in, rather than describing its format" fails — the pass-4 finding that this target passed off a stale shared-tmpdir file is closed, because `isolateTmp` in cliFixtures.ts now points TMPDIR/TEMP/TMP at the fixture's own directory for in-process and spawned runs alike; delete audit.ts:431, the `--proof N=` push, and "the generated pass file names every clause and is refused until its values are filled in" fails; delete audit.ts:626, step 8, and "makes logging tool friction a numbered step rather than a line to skip" fails; delete audit.ts:1120, the `nextAfterPass` print, and "tasks audit names the step that follows recording a pass" fails.
Live: the brief is a numbered 1-8 procedure printed above the data, and it wrote %TEMP%\audit-audit-brief-arrives-complete-pass5.txt with one `--proof N=`/`--evidence N=` pair for all eleven clauses, every value empty.
- proof 10: met — Both halves KILLED at narrow scope, no escalation: delete audit.ts:511, the `if (existsSync(path_)) return { kept: true };` in `writeArtifact`, and "keeps an artifact the auditor has already worked on rather than overwriting it" fails; replace audit.ts:602 with an unconditional `path.join(os.tmpdir(), ...)` and "offers no pass file either, in a brief that has just warned the diff is not this slugs" fails.
Live, both halves: with %TEMP%\mutations-audit-brief-arrives-complete.json aimed by hand and the pass file part-written, a second `audit-prompt audit-brief-arrives-complete` left both byte-identical and named each — "This file already exists and was left alone ... Delete it to regenerate against the current diff". `audit-prompt tasks-roadmap` prints "7. Do not file a pass", names no --args-from command, and wrote neither audit-tasks-roadmap-pass*.txt nor mutations-tasks-roadmap.json into %TEMP% (checked by listing it after the run).
Coverage note the clause survives: the kept path is only asserted over the pass file — the fixture's demo-spec has no proof targets, so `mutationManifest` returns zero entries and the manifest is never written in any test. The shared `writeArtifact` is what the mutation kills, so the mechanism is proved; the manifest call site is not.
- proof 11: met — Four targets KILLED at narrow scope, no escalation (1 failed of 131), in the 28-entry run: delete scripts/mutate.ts:536, the moved-HEAD arm of `recoveryStanding`, and "refuses to restore a journal captured at another commit, and says what it holds instead" fails; replace mutate.ts:487 with `head: journal.head as string,` and "treats a journal that records no commit, and a checkout git cannot answer for, as stale" fails on the readJournal assertion; replace mutate.ts:547 `if (read(file) !== text) write(file, text);` with an unconditional `write(file, text);` and "writes nothing and keeps nothing when no file was mutated, which is what a refused run leaves" fails; delete mutate.ts:549, the `failed.push(file)`, and "names a file it could not put back, which is the one reason to keep a journal" fails. Two extra probes outside the clause's targets — deleting mutate.ts:534 and mutate.ts:535 independently — were also KILLED, so all three arms of `recoveryStanding` are covered rather than only the last.
The give-the-tree-back half is measured live rather than read: `npm run mutate -- %TEMP%\mutations-audit-brief-arrives-complete.json` unaimed was refused for all 26 entries, and `ls %TEMP%\universalis-mutate-*` found nothing afterwards — the exact path that used to arm the next run. `git status --porcelain` was empty before and after, and the 28-entry run reported "The tree gained nothing and lost nothing while this run held it."
Residual the clause does not cover, filed as a low finding: staleness is decided from HEAD alone, so a journal captured at the same commit an arbitrary time ago is still restored over the working tree.
- proof 9: met — All four targets KILLED at narrow scope, no escalation, in the 28-entry run: delete audit.ts:608, step 1, and "sends the auditor to the spec file rather than reprinting its sections" fails; delete audit.ts:580, the `Diff range:` print, and "does not teach git, having already printed the range" fails; replace `otherSpecs.slice(0, 2)` at audit.ts:660 with `otherSpecs` and "names one other spec to check the standing against, not every spec in the checkout" fails — the pass-4 finding that this target admitted its own regression is closed, the test now counting the slugs between `one of: ` and the parenthesised fallback rather than the word "spec"; replace audit.ts:150 with `printPriorArt(priorArt(arch.manifest, tasks, owned));` and "counts the closed claims in the brief rather than listing them, and still lists them for one path" fails.
Live: the brief is 319 lines, carries no `## Deliverable` prose and no `## Decisions` section, prints no git command, names two other slugs plus `ls docs/specs`, and renders closed prior art as "67 closed claim(s) not listed" while `tasks where scripts/tasks/audit.ts` still lists every one.
Not closed by this clause, and still true at head: `parseSpecDoc` computes `deliverableProse` and `decisionsSection` for no reader anywhere in the tree (grep finds only the declaration and the assignment), which is the already-filed pass-4 unreviewed finding.
