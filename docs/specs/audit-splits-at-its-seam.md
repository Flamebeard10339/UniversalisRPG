...
- base: `8baf875525620db28aba8734bf8a3f7dcccc7431`
- head: `511c291af1f129f8721ed81c6951e3640d93bbf5`
- proof 1: met — grep -n "^export function cmdAuditPrompt\|^export function mutationManifest\|^export function resolveTarget\|^export function auditArgsSkeleton" scripts/tasks/audit.ts and grep -n "^export async function cmdAudit\|^export function cmdImport\|^export function parseAuditArgs" scripts/tasks/auditPrompt.ts both return no match (verified directly). Every brief-only name the clause lists by way of illustration (cmdAuditPrompt, mutationManifest, resolveTarget, auditArgsSkeleton, plus the suite-title index indexSuiteTitles/suiteFilesFor, the tool list AUDIT_TOOLS/toolLines, and the ownership survey printOwnership/diffChangedFiles) lives only in auditPrompt.ts; every filing-only name (cmdImport, parseAuditArgs, parseAuditFile, AUDIT_USAGE, walkClausesInteractively, buildFindingTask/filedFindings, cmdAudit) lives only in audit.ts. Additionally verified the move is pure and not a rewrite: a non-import, trimmed-line multiset diff between base scripts/tasks/audit.ts (8baf875, 1079 non-import lines) and the current audit.ts+auditPrompt.ts combined (1079 lines) found zero differing distinct lines, so no line was edited, duplicated or dropped in the move, and printOwnership/diffChangedFiles correctly went to the brief module per the open question (cmdAudit does not call them).
- proof 2: met — Base scripts/tasks/audit.test.ts at 8baf875 held 119 actual test declarations. A regex scan over it finds 123 it()/test() title-shaped matches; 4 are string-literal fixtures fed to testTitles() by the tests that exercise that parser ("a parameterised title", "a test that exists", "a title the fixture owns", and an escaped-apostrophe fixture) and are not test declarations — confirmed those exact strings already existed verbatim in the base file, so they are not new. After the split: npx vitest run scripts/tasks/audit.test.ts scripts/tasks/auditPrompt.test.ts -> "Test Files 2 passed (2)", "Tests 119 passed (119)" (65 in audit.test.ts, 54 in auditPrompt.test.ts, matching the vitest-reported per-file names exactly). The sorted multiset of all 123 title-shaped strings, fixtures included, is byte-identical between the base file and the current two files combined, so the 119 real test names and their count survive unchanged, none lost, renamed or duplicated.
- proof 3: met — grep -n "from './auditPrompt'" scripts/tasks/audit.ts -> line 6: import { resolveDiffRange } from './auditPrompt'; grep -n "from './audit'" scripts/tasks/auditPrompt.ts -> no match. Exactly one direction returns a line, which is the one-way import the Open Questions section sanctions: resolveDiffRange moved to the brief module and the filing module imports it (the second of the two options the spec offers). No other cross-reference exists between the files.
- proof 4: met — At base (8baf875) docs/audits/systems.json registered 4 concepts naming scripts/tasks/audit.ts: proof target resolution, generated auditor brief, clause deferral, record fault. After this branch, npm run audit-status's two-concept report shows scripts/tasks/audit.ts with 2 concepts (clause deferral, record fault) — strictly fewer than 4 — and scripts/tasks/auditprompt.ts with 3 (clause deferral, generated auditor brief, proof target resolution), i.e. "the rest". clause deferral correctly spans both files per the spec's own call (its wording is auditArgsSkeleton in auditPrompt.ts, its behaviour is walkClausesInteractively in audit.ts); record fault correctly stayed on audit.ts alone. Diffed the full two-concept path lists: base had 7 paths (src/grammar/action.ts, scripts/lib/taskstore.ts, scripts/lib/specdoc.ts, scripts/lib/producers.ts, scripts/tasks/records.ts, scripts/tasks/audit.ts, scripts/tasks/brieflessons.ts); current has the same 7 plus scripts/tasks/auditprompt.ts — no other path was added, and scripts/lib/taskStore.ts stayed at 5 concepts throughout, unaffected.
