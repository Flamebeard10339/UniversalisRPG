# CLAUDE.md snapshot (pre-DSL-teardown)

Verbatim copy of the project's `CLAUDE.md` as it stood on 2026-07-11, immediately
before the "Content pipeline" section and other DSL-specific references were
stripped from the live file as part of the DSL system teardown on the
`dsl-rewrite` branch. Kept here so the durable, non-DSL game-design principles
in this file aren't lost from view, and so the DSL-specific sections have a
home before they're deleted from the live document.
