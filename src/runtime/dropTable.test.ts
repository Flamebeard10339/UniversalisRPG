...
function fight(...body: string[]) {
  const registry = loadInEnglish([...ITEMS, '# stat luck', 'base: 60', '# stat ward', 'base: 60', '# item charm', 'food, +400 luck, 60s', '# flag lit', '# entity giant-rat', 'fight:', ...body.map((line) => `  ${line}`)].join('\n'));
