...
function carrying(holdings: Record<string, number>, over: Registry = registry): GameState {
  const state = initialState(over);
  for (const [id, count] of Object.entries(holdings)) receiveItem(state, over, id, count);
  return state;
}

function withGrownBlade(): GameState {
  return carrying({ 'heartwood-blade': 1 });
}
...
describe('what the screen lists', () => {
  it('lists stacks by title and count, and each grown copy under its own name', () => {
    const state = carrying({ 'heartwood-blade': 1, 'iron-sword': 3 });
...
describe('what the screen asks', () => {
  ...
  it('offers only the verbs the chosen item takes', () => {
    ...
    expect(values({ item: 'iron-sword' }, state, 'verb')).toEqual(['equip', 'destroy', LEAVE]);
  });
  ...
  it('offers a worn entry Unequip rather than an Equip that would do nothing', () => {
    ...
    expect(values({ item: 'worn:mainhand' }, state, 'verb')).toEqual(['unequip', 'destroy', LEAVE]);
    expect(values({ item: 'iron-sword' }, state, 'verb')).toEqual(['equip', 'destroy', LEAVE]);
    expect(values({ item: '1' }, state, 'verb')).toEqual(['grow', 'equip', 'destroy', LEAVE]);
  });
  ...
  it('offers Unequip to the very copy in the slot and Equip to the one beside it', () => {
    ...
    expect(values({ item: '2' }, state, 'verb')).toEqual(['grow', 'equip', 'destroy', LEAVE]);
  });
...
describe('what the screen does with an answer', () => {
  ...
  it('keeps the item it was answered with and asks the next question, rather than closing half-answered', () => {
    ...
    expect(carriedSubmit({ item: 'rope', verb: 'destroy' }, carrying({ rope: 1 }), registry)).toBeNull();
  });
  ...
  it('destroys a stack copy at once and a grown copy only once it is confirmed', () => {
    const state = carrying({ 'heartwood-blade': 1, rope: 2 });

    expect(carriedSubmit({ item: 'rope', verb: 'destroy' }, state, registry)).toBeNull();
    expect(packedCount(state, 'rope')).toBe(1);
  ...
  it('opens the plane of the copy in the slot, which is listed under its own id and not under the slot', () => {
    const state = carrying({ 'heartwood-blade': 2 });
    equip(state, registry, '2');

    expect(carriedSubmit({ item: '2', verb: 'grow' }, state, registry)).toEqual(planeFrame('2'));
    expect(carriedSubmit({ item: 'worn:mainhand', verb: 'grow' }, state, registry)).toBeNull();
