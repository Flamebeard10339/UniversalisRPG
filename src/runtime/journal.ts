...
const standingOf = (at: QuestStage, started: boolean): QuestStanding => (at.complete === true ? 'complete' : started ? 'started' : 'unstarted');
...
function entryFor(registry: Registry, state: GameState, quest: Quest): JournalEntry | null {
  ...
  return { quest: quest.id as Answer, title: localizer.title('quest', quest.id), stage: at.name as Answer, standing, lines, hint: standing === 'started' ? spoken(localizer, state, at.hint) : null };
}
