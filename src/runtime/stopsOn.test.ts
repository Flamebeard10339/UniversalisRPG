...
describe('an action ends on the events it names, and on none it does not', () => {
  it('runs the whole five seconds and banks all five payouts where it names nothing', () => {
    const state = ran('train', 5);
    expect(state.xp.lore).toBe(5 * PAYOUT);
    expect(state.activeAction).not.toBeNull();
  });

  it('stops at the first level-up, one payout in, where it names the event that fires there', () => {
    const state = ran('train-until-level', 5);
    expect(state.xp.lore).toBe(PAYOUT);
