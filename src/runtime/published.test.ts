...
const EN_ROUTE: ReadonlyArray<{ type: string; why: string }> = [
  ...
  { type: 'src/runtime/listedScreen.ts#Listed', why: 'what `listedScreen` builds from that declaration. Its `stale` hands one reason back to `pruneModals` and its `options` hands the same `ModalOption` list every other screen does, so both reach a player through the walk that already covers them' },
];
