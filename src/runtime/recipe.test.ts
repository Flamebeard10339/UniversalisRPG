...
describe('spannable repeating craft', () => {
  it('craft() on a time>0 recipe fires one completion then leaves an activeAction a later resolve() continues', () => {
    ...
    expect(state.activeAction).toEqual({ ownerRef: 'recipe.brick', actionLabel: CRAFT_LABEL, repeating: true, implicitTarget: toMilliUnits(1), cadences: { player: { progress: 0, attemptsMade: 0 } }, roster: { player: { ownerRef: 'recipe.brick', actionLabel: CRAFT_LABEL, target: 'brick' } } });
