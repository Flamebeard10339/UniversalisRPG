...
describe('a slot is a word with a key (c10)', () => {
  ...
  it('takes the words a # slot declares, and keeps the id the vocabulary named', () => {
    ...
    expect(() => loadModule(lines(ISLAND, '', '# slot head', 'title: Head', '', '# item hat', 'slot: head'))).toThrow(
  ...
  it('reaches a # locale, which is the whole of what the finding asked for', () => {
    const spanish: ModuleSource = { name: 'es', text: lines('# info island-es', 'version: 1.0.0', 'dependencies:', '  island', '', '# locale es', 'slot.mainhand.title: Mano principal') };
  ...
  it('prints a declared slot back out and drops a generated title, the way a stat does', () => {
    const source: ModuleSource = { name: 'island', text: lines(ISLAND, '', '# slot mainhand', 'title: Main Hand', '', '# slot offhand') };
