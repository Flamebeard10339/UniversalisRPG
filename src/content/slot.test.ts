...
const titleOf = (registry: Registry, slot: string): string | undefined =>
  registry.locales.base.get(localeKey(null, "slot", slot, "title"))?.text;
...
describe("a slot is a word with a key (c10)", () => {
  ...
  it("takes the words a # slot declares, and keeps the id the vocabulary named", () => {
    const registry = loadModule(
      lines(ISLAND, "", "# slot mainhand", "title: Main Hand"),
    );
    ...
    expect(() =>
      ...
      /# item island.hat slot: names head, which no # entity declares among its equipment-slots:/,
    );
  });
  ...
  it("gives one slot one key however many entities declare it", () => {
    ...
    expect(
      [...loadModule(shared).locales.addressable].filter((key) =>
        key.startsWith("slot.mainhand."),
      ),
    ).toEqual(["slot.mainhand.title"]);
  });
  ...
  it("reaches a # locale, which is the whole of what the finding asked for", () => {
    const spanish: ModuleSource = {
      ...
      text: lines(
        ...
        "",
        "# locale es",
        "slot.mainhand.title: Mano principal",
      ),
    };
    const registry = loadUniverse([{ name: "island", text: ISLAND }, spanish]);

    expect(
      registry.locales.declared.get("es")?.get("slot.mainhand.title"),
    ).toBe("Mano principal");
    // And an untranslated one is reported, rather than reaching a player as an
    // id nobody can read.
    expect(missingTranslations(registry.locales, "es")).toContain(
      "slot.offhand.title",
    );
  });
  ...
  it("prints a declared slot back out and drops a generated title, the way a stat does", () => {
    ...
    expect(trip.printed).toContain(
      lines("# slot mainhand", "title: Main Hand"),
    );
