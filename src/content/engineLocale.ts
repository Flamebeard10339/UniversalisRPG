...
export const withEngineLocale = (
  sources: readonly ModuleSource[],
): ModuleSource[] => {
  const engine = engineLocale();
  return sources.some((source) => source.name === engine.name)
    ? [...sources]
    : [engine, ...sources];
};
...
export const loadInEnglish = (source: string): Registry =>
  loadUniverse(withEngineLocale([{ name: "anonymous", text: source }]));
