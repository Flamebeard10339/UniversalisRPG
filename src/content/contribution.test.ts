...
describe("contribution issue packaging", () => {
  ...
  it("refuses to build a body whose notes carry the delimiter, before it is submitted", () => {
    ...
    expect(() => build("### local changes dsl")).toThrow(
      /notes cannot contain a Local Changes DSL heading/,
    );
    expect(() =>
      build(
        [
          "Quoting the shape of it:",
          "```md",
          "## Local Changes DSL",
          "```",
        ].join("\n"),
  ...
  it("reads back the content files it recorded, so a maintainer knows the base", () => {
    ...
    expect(contributionBase(body).contentFiles).toEqual([
      "content/base.dsl",
      "content/extra.dsl",
    ]);
  ...
  it("refuses a body with no delimiter rather than taking the first fence it finds", () => {
    ...
    expect(() => extractContributionDsl(body)).toThrow(
      /no Local Changes DSL heading/,
    );
  });
