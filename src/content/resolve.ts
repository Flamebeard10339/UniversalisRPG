...
function visibleTo(
  ...
  for (const dependency of module.info.dependencies) {
    if (dependency.prefix !== "incompatible" && loaded.has(dependency.module))
      visible.add(dependency.module);
  }
  return visible;
}
...
function missingOptionalDependencies(
  module: ParsedModule,
  loaded: ReadonlySet<string>,
): Set<string> {
  ...
  for (const dependency of module.info.dependencies) {
    if (
      ...
      missing.add(dependency.module);
  }
  return missing;
}

function namesMissingOptional(
  kind: string,
  raw: string,
...
function unorderedDependencies(module: ParsedModule): ReadonlySet<string> {
  return new Set(
    module.info.dependencies
      .filter((dependency) => dependency.prefix === "unordered")
...
function targetKey(
  ...
  const resolved = namespace.resolve(
    kind,
    id,
    module.namespace,
    visible,
  ...
  if (
    owner !== null &&
    owner !== undefined &&
    unorderedDependencies(module).has(owner)
  ) {
    throw new DslError(
      `# ${kind} ${id} edits ${owner}, but ~ dependencies do not load before this module. Use a load-order dependency for patches.`,
    );
  }
  return resolved;
}
...
function declareFlag(
  ...
  if (name === VISITS)
    throw new DslError(
      `${where} declares a flag named ${VISITS}, which the engine reads as a dialogue node's visit counter`,
    );
  return namespace.declareMember("flag", kind, id, name);
}

const addedMembers = <T>(value: unknown): T[] =>
  isFieldEdits(value)
    ? value.ops.filter((op) => op.op === "+").flatMap((op) => op.values as T[])
    : listMembers<T>(value);
...
export function actionAddresses(kind: string, value: MemberOwner): string[] {
  ...
  const inline = [
    ...addedMembers<Action>(value.actions),
    ...addedMembers<EntityBlock>(value.blocks).filter(
      (block) => !isHandlerBlock(block),
    ),
  ] as Action[];
  return [
    ...used,
    ...inline
      .filter((block) => !used.includes(lastSegment(block.label)))
      .map((block) => actionSlug(block.label)),
  ];
}
...
export function declareMembers(
  namespace: Namespace,
  kind: string,
  value: MemberOwner,
): Member[] {
...
const createdSections = (module: ParsedModule): Created[] =>
  module.sections.filter((section) => section.kind !== "remove") as Created[];
...
function declareIds(
  ...
  module.sections = module.sections.filter((section) => {
    if (section.kind === "remove")
      return !namesMissingOptional(
        (section.value as Removal).kind,
        (section.value as Removal).target,
        missingOptional,
      );
    const { id } = section.value as { id?: string };
    return (
      id === undefined ||
      !isNamespaced(section.kind) ||
      !id.includes(".") ||
      !namesMissingOptional(section.kind, id, missingOptional)
    );
  });

  for (const { kind, value } of createdSections(module)) {
    if (!isNamespaced(kind) || value.id === undefined || value.id.includes("."))
      continue;
    if (kind === "flag" && value.id === VISITS)
      throw new DslError(
        `# flag ${VISITS} is reserved: the engine reads <node>.${VISITS} as a dialogue node's visit counter`,
      );
...
function resolveReferences(
  ...
  for (const section of module.sections) {
    if (section.kind === "remove") {
      ...
      if (!isNamespaced(removal.kind))
        throw new DslError(
          `# remove ${removal.id}: ${removal.kind} is not a kind a module owns`,
        );
      removal.target = namespace.resolve(
        removal.kind,
        removal.target,
        self,
        visible,
      ...
      if (
        ...
        throw new DslError(
          `# remove ${removal.id} edits ${owner}, but ~ dependencies do not load before this module. Use a load-order dependency for patches.`,
        );
      }
    ...
    const visit = (kind: ReferenceKind, raw: string, where: string): string =>
      isNamespaced(kind) && !namesMissingOptional(kind, raw, missingOptional)
        ? namespace.resolve(kind, raw, self, visible, where, id)
        : raw;
...
export const RESOLUTION_PASSES: readonly ((
  module: ParsedModule,
  namespace: Namespace,
  loaded: ReadonlySet<string>,
) => void)[] = [declareIds, settleIds, resolveReferences];
