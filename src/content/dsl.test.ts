...
describe('every section kind', () => {
  it.each(sections().map((each) => each.kind))('%s declares a coherent section', (kind) => {
    ...
    // The bug this is aimed at shipped twice: a kind gained a title nobody
    // could translate, because the kind and the locale's list of prose fields
    // were two declarations of one fact. A minted key is allowed to have no
    // field behind it — `# recipe` translates a title it never parses — and a
    // field is allowed to hold words keyed elsewhere, which is what a `say:`
    // is. What may not happen is a title an author writes and nobody keys.
