...
describe('a second body written at an id a first body already holds', () => {
  ...
  // Which of its three branches `mergeFields` takes for one field, read off a merge rather than off the
  // schema: the page says what a second body does to a line, and this is the only thing that answers for it.
  ...
  // Whether a line goes home by the name it opens, read off the kind's own merge rather than off the mark
  // beside it: written twice under one name the body holds what one of it holds, and written under a
  // second name it holds more. Every line of every kind is asked, and the second name is the first one
  // renamed in place, so a mark put on a line that does not do this — or left off one that does — is
