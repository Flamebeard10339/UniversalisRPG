...
export function resolveDirective(
  ...
  visitDirective(directive, "typed directive", (kind, id, where) =>
    registry.namespace.resolve(kind, id, null, registry.namespace.all, where),
  );
  return directive;
}
...
export function resolveCarried(
  id: string,
  registry: Registry,
  where: string,
): string {
  if (mayBeInstanceId(id)) return id;
  return registry.namespace.resolve(
    "item",
    id,
    null,
    registry.namespace.all,
    where,
  );
}
