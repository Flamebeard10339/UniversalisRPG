...
const patch = (...lines: string[]): ModuleSource =>
  module("patch", "dependencies: base", ...lines);
...
describe("a section applies its fields over what the id already holds", () => {
  it("leaves the fields a patch does not list alone", () => {
    const item = loadUniverse([
      BASE,
      patch("# item base.rope", "examine: Frayed, but it holds."),
    ]).items.get("base.rope")!;
  ...
  it("does not reset unlisted fields to their defaults", () => {
    const entity = loadUniverse([
      BASE,
      patch("# entity base.crab", "examine: Bigger than you remember."),
    ]).entities.get("base.crab")!;
    expect(entity.title).toBe("Sand Crab");
    expect(entity.stats).toEqual({
      "base.attack": { min: 2, max: 4 },
      "base.defence": { min: 9, max: 9 },
    });
    expect(entity.actions.map((action) => action.label)).toEqual([
      "pinch",
      "flee",
    ]);
  });
  ...
  it("replaces a list field wholesale, because a bare key replaces", () => {
    const location = loadUniverse([
      BASE,
      patch("# location base.beach", "entities: base.crab"),
    ]).locations.get("base.beach")!;
    expect(location.entities).toEqual([{ entity: "base.crab" }]);
    expect(location.adjacent.map((edge) => edge.target)).toEqual([
      "base.dunes",
    ]);
...
describe("actions merge by label", () => {
  it("patches one field of one action and leaves its siblings whole", () => {
    const entity = loadUniverse([
      BASE,
      patch("# entity base.crab", "pinch:", "  time: 9"),
    ]).entities.get("base.crab")!;
    const pinch = entity.actions.find((action) => action.label === "pinch")!;
    expect(pinch.time).toBe(9);
    expect(pinch.results).toEqual([
      { kind: "say", text: "Ouch.", key: "base.entity.crab.say.0" },
    ]);
    expect(
      entity.actions.find((action) => action.label === "flee")!.results,
    ).toEqual([
      { kind: "say", text: "It scuttles off.", key: "base.entity.crab.say.1" },
    ]);
  });
  ...
  it("appends an action whose label is new, after the ones already there", () => {
    const entity = loadUniverse([
      BASE,
      patch("# entity base.crab", "wave:", "  say: It waves a claw."),
    ]).entities.get("base.crab")!;
    expect(entity.actions.map((action) => action.label)).toEqual([
      "pinch",
      "flee",
...
describe("a stat sheet is a list field", () => {
  const statsOf = (...lines: string[]) =>
    loadUniverse([BASE, patch("# entity base.crab", ...lines)]).entities.get(
      "base.crab",
    )!.stats;
...
describe("dialogue merges one node at a time", () => {
  ...
  const dialogueOf = (...lines: string[]) =>
    loadUniverse([withDialogue, patch(...lines)]).dialogues.get(
      "base.miki-intro",
    )!;

  it("replaces the steps of the node it names and leaves the others", () => {
    const dialogue = dialogueOf(
      "# dialogue base.miki-intro",
      "node greet:",
      "  Well met, traveller.",
    );
    expect(dialogue.nodes.map((node) => node.name)).toEqual([
      "greet",
      "farewell",
    ]);
...
describe("a list key takes + and -", () => {
  ...
  it("appends and removes independently, leaving the rest of the list alone", () => {
    expect(
      adjacency([BASE, reef("# location base.beach", "+adjacent: self.reef")]),
    ).toEqual(["base.dunes", "reef.reef"]);
    expect(
      adjacency([BASE, patch("# location base.beach", "-adjacent: dunes")]),
    ).toEqual([]);
  });
  ...
  it("removes a member named by only as much of it as identifies it", () => {
    ...
    expect(
      adjacency([
        conditional,
        patch("# location base.beach", "-adjacent: dunes"),
      ]),
    ).toEqual([]);
  });
  ...
  it("rejects replacing and modifying the same key in one section", () => {
    expect(() =>
      adjacency([
        BASE,
        patch("# location base.beach", "+adjacent: dunes", "adjacent: dunes"),
      ]),
    ).toThrow(/cannot be both replaced and modified/);
    expect(() =>
      adjacency([
        BASE,
        patch("# location base.beach", "adjacent: dunes", "-adjacent: dunes"),
  ...
  it("rejects an operator on a key that is not a list", () => {
    expect(() =>
      loadUniverse([BASE, patch("# item base.rope", "+title: Rope")]),
    ).toThrow(/title is not a list, so it cannot take \+/);
  });

  it("removes one action by label, and refuses a + that would mean nothing", () => {
    const entity = loadUniverse([
      BASE,
      patch("# entity base.crab", "-pinch:"),
    ]).entities.get("base.crab")!;
    expect(entity.actions.map((action) => action.label)).toEqual(["flee"]);
    expect(() =>
      loadUniverse([BASE, patch("# entity base.crab", "+wave:", "  say: Hi.")]),
    ).toThrow(/\+ means nothing here/);
  });
});
...
describe("# remove takes out what omission cannot", () => {
  it("removes a section, and complains when it names nothing", () => {
    const registry = loadUniverse([
      ...
      patch(
        "# location base.beach",
        "-adjacent: dunes",
        "# remove location.dunes",
      ),
    ]);
    expect(registry.locations.has("base.dunes")).toBe(false);
    expect(() =>
      loadUniverse([BASE, patch("# remove location.atlantis")]),
    ).toThrow(/names an unknown location: atlantis/);
  });
  ...
  it("rejects a reference to a removed item whichever module names it first", () => {
    const cut = (id: string): ModuleSource =>
      module(id, "dependencies: base", "# remove item.rope");
