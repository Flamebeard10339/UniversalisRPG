...
export function putCarried<T extends object>(
  holder: T,
  key: keyof T & string,
  where: string,
  visit: Visit,
...
export function segments(
  list: TextSegment[] | undefined,
  where: string,
  visit: Visit,
): void {
  for (const segment of list ?? []) {
    if (segment.kind === "conditional")
      condition(segment.condition, where, visit);
    if (segment.kind === "interpolate")
      reference(segment.reference, where, visit);
  }
}
...
export function quantified(
  ...
  for (const entry of listMembers<Quantified>(list))
    put(entry, "item", kind, where, visit);
}
    ...
    switch (result.kind) {
      ...
      case "contest":
        ...
        for (const side of ["left", "right"] as const)
          put(result, side, "stat", `${where} vs:`, visit);
        break;
      case "gate":
      ...
      case "pool":
        put(
          ...
          "resource",
          "resource",
          `${where} ${result.delta.max < 0 ? "drain" : "restore"}:`,
          visit,
        );
        break;
      case "set":
...
export function visitTags(list: unknown, where: string, visit: Visit): void {
  for (const tag of listMembers<TagClause>(list)) {
    ...
    if (tag.per !== undefined)
      put(
        tag.per,
        "id",
        tag.per.kind === "stack" ? "item" : "resource",
        `${where} tag per`,
        visit,
      );
  }
...
export function hooks(carrier: Loose, where: string, visit: Visit): void {
  ...
  results(
    listMembers<ActionResult>(carrier.whenHit),
    `${where} when hit:`,
    visit,
  );
}
...
function sidedNames(
  action: Action,
): { held: Sided; kind: ReferenceKind; written: string }[] {
  ...
  if (typeof action.rate === "object" && action.rate !== null)
    sites.push({ held: action.rate, kind: "stat", written: "rate" });
  for (const [written, contest] of [
    ...
    if (contest.right)
      sites.push({ held: contest.right, kind: "stat", written });
  }
  if (action.depletes)
    sites.push({
...
export function visitAction(action: Action, where: string, visit: Visit): void {
  for (const site of sidedNames(action))
    put(site.held, "id", site.kind, `${where} ${site.written}:`, visit);
  ...
  for (const group of [
    ...
    results(group, where, visit);
}

export function actions(list: unknown, where: string, visit: Visit): void {
  for (const action of listMembers<Action>(list))
    visitAction(
      action,
