...
describe("a module declares into its own namespace", () => {
  ...
  it("leaves ids at the root when a lone module declares no # info, and refuses that in company", () => {
    ...
    expect(() =>
      loadUniverse([{ name: "loose", text: "# item rope" }, BASE]),
    ).toThrow(/declares no # info, so its ids have no namespace/);
  });
});

describe("a reference may drop leading segments", () => {
  const referring = (reference: string) => () =>
    loadUniverse([
      BASE,
      module(
        "mod",
  ...
  const given = (reference: string) =>
    ...
    ]).entities.get("mod.gull")!.actions[0].results[0];
  ...
  it("refuses a module that is neither this one nor a declared dependency", () => {
    ...
    expect(() =>
      loadUniverse([
        BASE,
        stranger,
        module("mod", "# entity gull", "peck:", "  give: stranger.rope"),
      ]),
    ).toThrow(
  ...
  it("names both candidates rather than picking one", () => {
    ...
    expect(() => loadUniverse([BASE, shadowing])).toThrow(
      /ambiguous between base.rope and mod.rope/,
    );
  });
  ...
  it("lets self. pick this module out of the ambiguity", () => {
    ...
    expect(
      loadUniverse([BASE, shadowing]).entities.get("mod.gull")!.actions[0]
        .results[0],
    ).toEqual({ kind: "give", item: "mod.rope" });
  });
  ...
  it("is directed by the kind the site wants, so a same-named entity does not shadow an item", () => {
    ...
    expect(
      loadUniverse([BASE, both]).entities.get("mod.gull")!.actions[0]
        .results[0],
    ).toEqual({ kind: "give", item: "base.rope" });
  });
});
...
describe("a heading creates or edits by its shape alone", () => {
  it("creates in this module when bare, even though a dependency has that name", () => {
    const registry = loadUniverse([
      BASE,
      module("mod", "dependencies: base", "# item rope", "title: Nylon Rope"),
    ]);
  ...
  it("refuses to edit a path that names nothing", () => {
    ...
    ).toThrow(/names an unknown item: base.cable/);
  });
  ...
  it("refuses to edit a dependency declared with ~ because it does not load first", () => {
    ...
    expect(() => loadUniverse([patch, dependency])).toThrow(
      /~ dependencies do not load before this module/,
    );
  });
});
...
describe("a - op declares nothing", () => {
  ...
  const patch = (...lines: string[]) =>
    module("mod", "dependencies: base", "# entity base.rat", ...lines);

  it("leaves a flag that never existed undeclared, so a reference to it fails the way any typo does", () => {
    expect(() =>
      loadUniverse([RAT, patch("-flags: ghost", "poke:", "  requires: ghost")]),
    ).toThrow(/names an unknown flag: ghost/);
  });

  it("says nothing about a removal that matched nothing, when nothing refers to it", () => {
    expect(
      loadUniverse([RAT, patch("-flags: ghost")]).entities.get("base.rat")!
        .flags,
    ).toEqual(["alert"]);
  });

  it("still declares a name the same section adds after removing it, because the merge keeps it", () => {
    const patched = loadUniverse([
      RAT,
      patch("-flags: bolted", "+flags: bolted", "poke:", "  requires: bolted"),
    ]).entities.get("base.rat")!;
...
describe("a ~ dependency is visible whichever way the module names sort", () => {
  ...
  const referrer = (id: string, ...lines: string[]) =>
    module(id, "dependencies: ~ target", ...lines);

  it("resolves a reference into it, which is the whole point of ~", () => {
    for (const id of ["aref", "zref"]) {
      const registry = loadUniverse([
        referrer(id, "# entity npc", "rummage:", "  give: target.gem"),
        target,
      ]);
      expect(registry.entities.get(`${id}.npc`)!.actions[0].results[0]).toEqual(
        { kind: "give", item: "target.gem" },
      );
    }
  });
  ...
  it("still refuses an edit or a removal, and says why rather than that the id is unknown", () => {
    for (const id of ["aaa", "zzz"]) {
      expect(() =>
        loadUniverse([referrer(id, "# item target.gem", "title: Gem"), target]),
      ).toThrow(/~ dependencies do not load before this module/);
      expect(() =>
        loadUniverse([referrer(id, "# remove item.target.gem"), target]),
      ).toThrow(/~ dependencies do not load before this module/);
    }
  });
});
...
describe("names the engine keeps for itself", () => {
  it("refuses a flag called visits, which a condition reads as a node counter", () => {
    expect(() => loadUniverse([module("base", "# flag visits")])).toThrow(
      /# flag visits is reserved/,
    );
    expect(() =>
      loadUniverse([
        module("base", "# location camp", "x: 0, y: 0", "flags: visits"),
      ]),
    ).toThrow(/declares a flag named visits/);
  });

  it("reserves the module ids the engine actually owns", () => {
    for (const reserved of ["time", "player", "item", "self"]) {
      expect(() => loadUniverse([module(reserved, "# item rope")])).toThrow(
        /is a reserved module id/,
      );
    }
...
describe("what a namespace does not reach", () => {
  it("leaves a tuning variable global, because the engine reads it by name", () => {
    expect([
      ...loadUniverse([
        module("base", "# variable min-damage", "value: 3"),
      ]).variables.keys(),
    ]).toEqual(["min-damage"]);
  });
  ...
  it("leaves a station global, because it is a contract between modules that never met", () => {
    ...
    expect(
      loadUniverse([kitchen, cook]).recipes.get("cook.bake")!
        .requiresCapability,
    ).toBe("oven");
  });
...
describe("a new game begins in exactly one place", () => {
  it("refuses two starting locations rather than picking by source order", () => {
    ...
    ).toThrow(/is marked starting, and so is/);
  });
});
...
describe("an action's address is a member of the namespace", () => {
  ...
  it("hangs an inline block under the object that heads it, on all three kinds that own one", () => {
    expect(
      namespace().has(ACTION_MEMBER, "entity.isla.dresser.search-drawer"),
    ).toBe(true);
    expect(
      namespace().has(ACTION_MEMBER, "location.isla.shore.light-beacon"),
    ).toBe(true);
  ...
  it("hangs an action a `uses:` brings under the entity that brings it", () => {
    expect(namespace().has(ACTION_MEMBER, "entity.isla.dresser.pry")).toBe(
      true,
    );
  ...
  it("puts it beside the flags of the same owner, under the one path grammar", () => {
    ...
    expect(namespace().snapshot()).toContain(
      `${ACTION_MEMBER} entity.isla.dresser.search-drawer isla`,
    );
  });
