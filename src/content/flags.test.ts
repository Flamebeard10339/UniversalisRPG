...
const written = (source: string, entity = "door"): string => {
  ...
  return (action.onSuccess ?? action.results)[0].kind === "set"
    ? ((action.onSuccess ?? action.results)[0] as { variable: string }).variable
    : "";
};
...
describe("a flag is declared by whatever owns it", () => {
  it("hangs a module flag under the module, so no prefix has to be spelled by hand", () => {
    ...
    expect(
      registry.entities.get("tutorial.oven")!.actions[0].results[0],
    ).toEqual({ kind: "set", variable: "tutorial.made-bread" });
  });

  it("hangs an object flag under the object that lists it", () => {
    expect(
      written(
        ["# entity door", "flags: unlocked", "pick:", "  set: unlocked"].join(
          "\n",
        ),
      ),
    ).toBe("door.unlocked");
  ...
  it("refuses a flag nobody declared, which is where a typo used to read false forever", () => {
    expect(() =>
      loadModule(
        ["# entity door", "flags: unlocked", "pick:", "  set: unlokced"].join(
          "\n",
        ),
      ),
    ).toThrow(/names an unknown flag: unlokced/);
    expect(() =>
      loadModule(["# entity door", "peek:", "  requires: unlocked"].join("\n")),
...
describe("a bare name means the flag of the object it was written in", () => {
  ...
  it("names both candidates when the reference is written outside either of them", () => {
    ...
    expect(() => loadModule(outside)).toThrow(
      /ambiguous between door.unlocked and gate.unlocked/,
    );
  });

  it(`reads another object's flag by naming that object, which is what a bare one could not do`, () => {
    const registry = loadModule(
      [TWO_DOORS, "# entity thief", "gloat:", "  requires: gate.unlocked"].join(
        "\n",
      ),
    );
...
describe("what the engine answers itself", () => {
  const asks = (condition: string): string[] => {
    const registry = loadModule(
      ["# entity sage", "muse:", `  requires: ${condition}`].join("\n"),
    );
...
describe("a dialogue node is a member too, and its visits are counted against its path", () => {
  ...
  const asked = (source: string, dialogue = "miki"): string[] => {
    ...
    return (
      (node.when as { left: { path: string[] } }).left ??
      (node.when as unknown as { reference: { path: string[] } }).reference
    ).path;
  };
...
describe("a flag across module boundaries", () => {
  ...
  it("is reached by a dependency without naming the module, and by path when shadowed", () => {
    const registry = loadUniverse([
      ...
      module(
        ...
        "# entity gull",
        "squawk:",
        "  requires: tide-out and door.unlocked",
      ),
    ]);
  ...
  it("is refused when the module was never declared a dependency", () => {
    expect(() =>
      loadUniverse([
        BASE,
        module("mod", "# entity gull", "squawk:", "  requires: base.tide-out"),
      ]),
    ).toThrow(/but base is not this module or one of its dependencies/);
  });
  ...
  it(`can be added to another module's object, and hangs under that object`, () => {
    ...
    expect(registry.entities.get("base.door")!.flags).toEqual([
      "unlocked",
      "bolted",
    ]);
  ...
  it("goes away with the object it belonged to", () => {
    ...
    const wrecker = module(
      "wrecker",
      "dependencies: doomed",
      "# remove entity.doomed.door",
      "# entity gull",
      "squawk:",
      "  requires: door.unlocked",
    );
    expect(() => loadUniverse([doomed, wrecker])).toThrow(
      /names an unknown flag: doomed.door.unlocked/,
    );
  });
});
...
describe("a flag a field edit takes away", () => {
  ...
  const cut = (id: string): ModuleSource =>
    module(id, "dependencies: base", "# entity base.door", "-flags: unlocked");
  const wants = (id: string): ModuleSource =>
    module(
      id,
      "dependencies: base",
      "# entity gull",
      "squawk:",
      "  requires: base.door.unlocked",
    );
  ...
  it("goes away with the value, so a reference the edit stranded no longer resolves", () => {
    expect(() => loadUniverse([BASE, cut("mod"), wants("watcher")])).toThrow(
      dangles,
    );
  });
  ...
  it("fails whichever module names it first, because what survives is decided at merge", () => {
    expect(() =>
      loadUniverse([BASE, cut("aaa-cut"), wants("zzz-wants")]),
    ).toThrow(dangles);
    expect(() =>
      loadUniverse([BASE, wants("aaa-wants"), cut("zzz-cut")]),
    ).toThrow(dangles);
  });
  ...
  it("stays when a + in the same section puts it back, because the merged section is what is asked", () => {
    ...
    expect(
      loadUniverse([BASE, readd]).entities.get("base.door")!.flags,
    ).toEqual(["unlocked"]);
  });
  ...
  it("leaves discovered alone when a location edits its flags, because a location owns a member it never lists", () => {
    ...
    const strip = module(
      ...
      "# entity gull",
      "squawk:",
      "  requires: base.beach.discovered",
    );
...
describe("a member key is owned by every kind that declares it", () => {
  ...
  it("keeps a location discoverable when an entity of the same id edits discovered away", () => {
    ...
    expect(
      loadUniverse([shadow, strip]).namespace.has(
        "flag",
        "base.beach.discovered",
      ),
    ).toBe(true);
  });
  ...
  it("goes away when the module that added it is not the one that created the object", () => {
    ...
    expect(
      loadUniverse([door, adds, cuts]).namespace.has(
        "flag",
        "base.door.sealed",
      ),
    ).toBe(false);
    expect(
      loadUniverse([door, adds]).namespace.has("flag", "base.door.sealed"),
    ).toBe(true);
  });
});
...
describe("an action a field edit takes away", () => {
  ...
  const cut = (id: string): ModuleSource =>
    module(id, "dependencies: base", "# entity base.dresser", "-uses: pry");
  const wants = (id: string): ModuleSource =>
    module(
      id,
      "dependencies: base",
      "# test walk",
      "use: entity.base.dresser.pry",
    );
  ...
  it("takes an inline block away with the block, not only with the object that headed it", () => {
    ...
    expect(
      loadUniverse([shelf, cutBlock]).entities.get("base.shelf")!.actions,
    ).toEqual([]);
    expect(() => loadUniverse([shelf, cutBlock, dusts])).toThrow(
      /names an unknown action-slug: entity.base.shelf.dust-it/,
    );
  });
  ...
  it("takes it with the object too, when the whole object goes", () => {
    expect(() =>
      loadUniverse([
        shelf,
        module("mod", "dependencies: base", "# remove entity.base.shelf"),
        dusts,
      ]),
    ).toThrow(/names an unknown entity: base.shelf/);
  });
});
