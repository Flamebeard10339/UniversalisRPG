...
function fencedDsl(lines: readonly string[]): string {
  ...
  if (start === -1)
    throw new Error(
      `the ${DSL_HEADING_LABEL} section contains no ${DSL_FENCE} block`,
    );
  const rest = lines.slice(start + 1);
  const end = rest.findIndex((line) => line.startsWith(FENCE));
  if (end === -1)
    throw new Error(
      `the ${DSL_HEADING_LABEL} section has an unterminated ${DSL_FENCE} block`,
    );
...
function validationLines(validation: UniverseLoadResult): string[] {
  const lines = [
    `Loaded modules: ${validation.loadedModules.join(", ") || "(none)"}`,
  ];
  if (validation.disabledModules.length > 0)
    lines.push(`Disabled modules: ${validation.disabledModules.join(", ")}`);
  if (validation.diagnostics.length === 0) lines.push("Diagnostics: none");
  else
    lines.push(
      "Diagnostics:",
      ...validation.diagnostics.map(
        (diagnostic) => `- ${formatModuleDiagnostic(diagnostic)}`,
      ),
    );
  return lines;
}
...
export function localDiagnostics(
  ...
  return diagnostics.filter(
    (diagnostic) =>
      ...
      diagnostic.moduleId === LOCAL_CHANGES_MODULE_ID,
  );
}

export function localModuleLoaded(
  sourceName: string,
  validation: UniverseLoadResult,
): boolean {
  const status = validation.modules.find(
    (module) => module.sourceName === sourceName,
  );
  return (
    status?.moduleId === LOCAL_CHANGES_MODULE_ID &&
    status.loaded === true &&
    localDiagnostics(sourceName, validation.diagnostics).length === 0
  );
}

export function buildContributionIssueBody(
  input: ContributionIssueInput,
): string {
  ...
  if (issueSections(notes).some((section) => section.label === DSL_HEADING)) {
    throw new Error(
      `contributor notes cannot contain a ${DSL_HEADING_LABEL} heading: it is the delimiter the module is read from`,
    );
  }
...
export function extractContributionDsl(issueBody: string): string {
  const matching = issueSections(issueBody).filter(
    (section) => section.label === DSL_HEADING,
  );
  if (matching.length === 0)
    throw new Error(`issue body has no ${DSL_HEADING_LABEL} heading`);
  if (matching.length > 1)
    throw new Error(
      `issue body has ${matching.length} ${DSL_HEADING_LABEL} headings, so which block is the module is ambiguous`,
    );
