...
const fromSchemas = () =>
  reachableCodecs(
    Object.entries(SCHEMAS).flatMap(([kind, schema]) =>
      Object.entries((schema as AnySchema).fields).map(
        ([field, spec]) => [`${kind}.${field}`, spec.parser] as const,
      ),
    ),
  );
...
describe("every parser in the grammar is a codec that survives its own examples", () => {
  ...
  it("reaches the parsers only a schema names, and finds each one round-tripping", () => {
    ...
    expect(
      [...codecs.keys()].some((parser) => !exported.has(parser as never)),
    ).toBe(true);
  });
  ...
  it("finds a codec behind every field of every collected kind", () => {
    const fields = Object.entries(SCHEMAS).flatMap(([kind, schema]) =>
      Object.entries((schema as AnySchema).fields).map(
        ([field, spec]) => [`${kind}.${field}`, spec.parser] as const,
      ),
    );
    ...
    expect(
      fields.filter(([, parser]) => !isCodec(parser)).map(([name]) => name),
    ).toEqual([]);
  });
});
...
const FIXTURE = readFileSync(
  path.join(import.meta.dirname, "printedCorpus.fixture.txt"),
  "utf8",
);
...
describe("nothing that loads today loads differently, and nothing prints differently", () => {
  const printCorpus = () => {
    ...
    const trip = roundTripUniverse(
      ...
      parseUniverse(sources).filter(canSerialize),
  ...
  it("prints every shipped module to the bytes it printed to at the branch base", () => {
    expect(
      printCorpus()
        .map((source) => `${SEPARATOR}${source.name}\n${source.text}`)
        .join(""),
    ).toEqual(FIXTURE);
  });
...
const reloadAlone = (printed: string): UniverseLoadResult =>
  loadUniverseWithDiagnostics([{ name: "base", text: printed }]);

const diagnostic = (message: string): ModuleDiagnostic => ({
  sourceName: "base",
  moduleId: "base",
  stage: "validate",
  message,
...
describe("roundTripModule", () => {
  ...
  it.each([
    ...
    "reports a hook the reload lost, so dropping one cannot read as clean: %s",
    (printedForm, where, id) => {
      const result = roundTripModule(
        loadModule(CARRIERS),
        { info },
        (printed) => {
  ...
  it("names what the reloaded registry lost, so a dropped section cannot read as clean", () => {
    const result = roundTripModule(loadModule(BASE), { info }, (printed) =>
      reloadAlone(without(printed, "# item bread")),
    );
  ...
  it("reports a value the reload changed, not only one it dropped", () => {
    const result = roundTripModule(loadModule(BASE), { info }, (printed) =>
      reloadAlone(printed.replace("x: 0, y: 0", "x: 1, y: 0")),
    );
  ...
  it("stops at diagnostics and does not diff a registry that never loaded", () => {
    const failed = {
      registry: loadModule("# info base\nversion: 1.0.0\n"),
      diagnostics: [diagnostic("did not validate")],
      modules: [],
      parsed: [],
      loadedModules: [],
      disabledModules: [],
    ...
    expect(result.diagnostics.map((each) => each.message)).toEqual([
      "did not validate",
    ]);
  ...
  it("returns the printed text even when the reload rejected it", () => {
    const result = roundTripModule(loadModule(BASE), { info }, () => ({
      registry: loadModule("# info base\nversion: 1.0.0\n"),
      diagnostics: [diagnostic("nope")],
      modules: [],
      parsed: [],
      loadedModules: [],
      disabledModules: [],
  ...
  it("carries the serializer options through, and reports the variable dropped when they are absent", () => {
    const withVariable = loadModule(
      [BASE, "", "# variable travel-seconds-per-unit", "value: 5"].join("\n"),
    );
    const undeclared = roundTripModule(withVariable, { info }, reloadAlone);
    expect(undeclared.printed).not.toContain("# variable");
    expect(undeclared.differences).toEqual([
      "  variables: missing travel-seconds-per-unit",
    ]);

    const declared = roundTripModule(
      withVariable,
      { info, globals: ["travel-seconds-per-unit"] },
      reloadAlone,
    );
...
describe("republishModule", () => {
  const renamed = {
    info: { id: "again", version: [1, 0, 0] as [number, number, number] },
  };

  it("prints under the new id once the round trip under the old one came back clean", () => {
    const result = republishModule(loadModule(BASE), { info }, reloadAlone, {
  ...
  it("prints nothing, and says what was lost, when the round trip found a difference", () => {
    const result = republishModule(
      loadModule(BASE),
      { info },
      (printed) => reloadAlone(without(printed, "# item bread")),
      {
        registry: loadModule(BASE),
        options: renamed,
      },
  ...
  it("prints nothing when the round trip did not load, and carries the diagnostic", () => {
    const failed: UniverseLoadResult = {
      registry: loadModule(["# info base", "version: 1.0.0", ""].join("\n")),
      diagnostics: [diagnostic("nope")],
      modules: [],
      parsed: [],
      loadedModules: [],
    ...
    const result = republishModule(loadModule(BASE), { info }, () => failed, {
      registry: loadModule(BASE),
      options: renamed,
    });
    expect(result.printed).toBeNull();
...
describe("declaredGlobalIds", () => {
  it("answers in a stable order, so the serializer prints the same bytes whatever order they were authored in", () => {
    const one = parseUniverse([
      {
        name: "m",
        text: "# info m\nversion: 1.0.0\n\n# variable zebra\nvalue: 1\n\n# variable alpha\nvalue: 2\n",
      },
    ])[0];
    const other = parseUniverse([
      {
        name: "m",
        text: "# info m\nversion: 1.0.0\n\n# variable alpha\nvalue: 2\n\n# variable zebra\nvalue: 1\n",
      },
...
describe("canSerialize", () => {
  it("is false for a source with no # info, whose ids no namespace prefix matches", () => {
    expect(
      canSerialize(
        parseUniverse([{ name: "snippet", text: "# item rock\n" }])[0],
      ),
    ).toBe(false);
  });
...
describe("roundTripUniverse", () => {
  ...
  const reloading =
    (
      transform: (
        printed: readonly ModuleSource[],
      ) => readonly ModuleSource[] = (printed) => printed,
    ) =>
    (printed: readonly ModuleSource[]) => {
      handed.push([...printed]);
  ...
  const trip = (sources: ModuleSource[]) => {
    ...
    return roundTripUniverse(
      loaded.registry,
      parseUniverse(sources),
      reloading(),
    );
  };
  ...
  it("survives a universe where one module patches another module's content", () => {
    ...
    expect(
      result.sources.find((source) => source.name === "base")?.text,
    ).toContain("Toast");
  });
  ...
  it("carries a hook a patch module emptied", () => {
    ...
    expect(loaded.registry.entities.get("base.rat")).toMatchObject({
      onHit: [],
      whenHit: [],
    });
  ...
  it("still reports a real loss, so the reframing did not make it blind", () => {
    ...
    const result = roundTripUniverse(
      loaded.registry,
      parseUniverse([BASE_TWO]),
      reloading((printed) =>
        printed.map((source) => ({
          ...source,
          text: source.text
            .split("\n\n")
            .filter((section) => !section.startsWith("# item rope"))
            .join("\n\n"),
        })),
      ),
    );
