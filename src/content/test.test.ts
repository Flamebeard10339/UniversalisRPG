...
describe("test: composable in-game scripts", () => {
  ...
  it("parses an assert with a comparison and a negated condition", () => {
    const source = [
      "# test rat-hunt",
      "assert: skills.combat.visits >= 3",
      "assert: not tutorial.snubbed",
    ].join("\n");
  ...
  it("accepts fully-qualified names emitted by CLI authoring and recording", () => {
    ...
    expect(section.value.directives).toEqual([
      ...
      { kind: "talk", entity: "tutorial-island.miki" },
      {
        kind: "use",
        obj: "entity",
        objId: "tutorial-island.front-door",
        actionId: "pick-lock",
      },
      ...
      { kind: "expect", save: "tutorial-island.end" },
      {
        ...
        inner: {
          kind: "use",
          obj: "entity",
          objId: "tutorial-island.oven",
          actionId: "roast-chestnuts",
        },
      },
      {
        kind: "begin",
        inner: { kind: "travel", location: "tutorial-island.basement" },
      },
      {
        kind: "begin",
        inner: { kind: "craft", recipe: "tutorial-island.dough" },
      },
    ]);
  });
...
describe("open-modal: raises a screen by name", () => {
  ...
  it("reads a payload that is no name at all as no directive", () => {
    const bad = (line: string) => () =>
      parseModule(["# test bad", line].join("\n"));
    expect(bad("open-modal: Carried Items")).toThrow(
      /unexpected line in # test/,
    );
...
describe("submit-modal: answers one option of the open modal", () => {
  ...
  it("names the offending line for a payload that is not key=value, rather than reading as an unknown directive", () => {
    expect(() => parseModule("# test bad\nsubmit-modal: Rowan")).toThrow(
      /malformed submit-modal: payload \(expected <key>=<value>\): submit-modal: Rowan/,
    );
    expect(() => parseModule("# test bad\nsubmit-modal:")).toThrow(DslError);
    expect(() => parseModule("# test bad\nsubmit-modal: Name=Rowan")).toThrow(
      /malformed submit-modal:/,
    );
  });
});
...
describe("the four growth verbs", () => {
  ...
  it("takes a target spelled as an item id or as a minted instance id, and nothing else", () => {
    ...
    expect(() => parsed("feed: 1a with whetstone")).toThrow(
      /malformed feed: payload/,
    );
  });

  it("names the offending line for a malformed payload rather than reading as an unknown directive", () => {
    expect(() => parsed("feed: heartwood-blade")).toThrow(
      /malformed feed: payload \(expected <target> with <item>\)/,
    );
    expect(() => parsed("slot: 1 at 0,0 with node-jewel")).toThrow(
      /malformed slot: payload/,
    );
    expect(() => parsed("allocate: 1 at 0,0 position e")).toThrow(
      /malformed allocate: payload/,
    );
    expect(() => parsed("allocate: 1 at 0,0 slot up")).toThrow(
      /malformed allocate: payload/,
    );
    expect(() => parsed("apply: 1 with lesser-orb")).toThrow(
      /malformed apply: payload/,
    );
  });
  ...
  it("refuses a hex address the plane would not have written", () => {
    expect(() => parsed("apply: 1 at 01,0 with lesser-orb")).toThrow(
      /malformed hex address/,
    );
    expect(() => parsed("apply: 1 at -0,0 with lesser-orb")).toThrow(
      /malformed hex address/,
    );
  });
});
...
describe("refuse: the outcome under test", () => {
  ...
  it("rejects a verb whose refusal is not a value the plane returns", () => {
    expect(() => parseDirectiveLine("refuse: travel beach")).toThrow(
      /unknown refuse: verb \(expected one of feed, slot, allocate, apply\)/,
    );
    expect(() => parseDirectiveLine("refuse: feed 1")).toThrow(
      /malformed feed: payload/,
    );
  });
});
