...
function allEdgesTouch(position: number): Record<Direction, number> {
  return { e: position, ne: position, nw: position, w: position, sw: position, se: position };
}
...
const RING_EDGES: Readonly<Record<Direction, number>> = { w: 1, nw: 2, ne: 3, e: 4, se: 5, sw: 6 };
