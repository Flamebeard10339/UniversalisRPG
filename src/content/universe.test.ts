...
const ids = (sources: ModuleSource[]): string[] =>
  parseUniverse(sources).map((parsed) => parsed.info.id);

describe("module identity", () => {
  it("reads id, version and pack off # info", () => {
    const parsed = parseModuleSource(
      module("whatever", "# info orc-pack", "version: 2.1", "pack: bestiary"),
    );
  ...
  it("falls back to the source name, with a zero version and no dependencies", () => {
    const parsed = parseModuleSource(
      module("tutorial-island", "# stat attack"),
    );
  ...
  it("rejects a second # info, a reserved id, and a duplicate id", () => {
    expect(() =>
      parseModuleSource(module("m", "# info a", "# info b")),
    ).toThrow(/declares # info more than once/);
    expect(() => parseModuleSource(module("m", "# info entity"))).toThrow(
      /entity is a reserved module id/,
    );
    expect(() => parseModuleSource(module("m", "# info player"))).toThrow(
      /player is a reserved module id/,
    );
    expect(() =>
      orderModules([
        parseModuleSource(module("a", "# info a")),
        parseModuleSource(module("a", "# info a")),
      ]),
    ).toThrow(/two modules declare the id a/);
  });
});
...
describe("load order", () => {
  it("loads a dependency before the module that declares it", () => {
    expect(
      ids([
        module("zebra", "# info zebra", "dependencies: alpha"),
        module("alpha", "# info alpha"),
      ]),
    ).toEqual(["alpha", "zebra"]);
  });
  ...
  it("places a module immediately after the dependency that unblocks it", () => {
    const sources = [
      module("b", "# info b"),
      module("a-late", "# info a-late", "dependencies: b"),
      module("c", "# info c"),
    ];
  ...
  it("makes the later module win a conflict, so the order is visible in the registry", () => {
    ...
    expect(
      loadUniverse([second, first]).stats.get("alpha.attack")!.base,
    ).toEqual({ min: 9, max: 9 });
  });
  ...
  it("builds the same universe from the same modules in any source order", () => {
    ...
    const bestiary = module(
      "bestiary",
      "# info bestiary",
      "dependencies: addon, base",
      "# entity ogre",
      "stats: attack 4-7",
    );
    ...
    for (const sources of sourceOrders) {
      ...
      expect(universe.entities.get("bestiary.ogre")!.stats).toEqual(
        expected.ogre,
      );
    }
  });
...
describe("dependency declarations", () => {
  it("requires a hard dependency to be present", () => {
    expect(() =>
      ids([module("m", "# info m", "dependencies: missing")]),
    ).toThrow(/names a module that is not loaded: missing/);
  });

  it("loads without an absent optional or recommended dependency", () => {
    expect(
      ids([module("m", "# info m", "dependencies: ? missing, + also-missing")]),
    ).toEqual(["m"]);
  });
  ...
  it("orders against an optional dependency that is present", () => {
    expect(
      ids([
        module("a-late", "# info a-late", "dependencies: ? zebra"),
        module("zebra", "# info zebra"),
      ]),
    ).toEqual(["zebra", "a-late"]);
  });

  it("rejects a loaded incompatible module", () => {
    expect(() =>
      ids([
        module("m", "# info m", "dependencies: ! other"),
        module("other", "# info other"),
      ]),
    ).toThrow(/m is incompatible with other/);
  });

  it("checks the version of a dependency that is present", () => {
    const base = module("base", "# info base", "version: 1.2.0");
    expect(
      ids([module("m", "# info m", "dependencies: base >= 1.2"), base]),
    ).toEqual(["base", "m"]);
    expect(
      ids([module("m", "# info m", "dependencies: base < 2"), base]),
    ).toEqual(["base", "m"]);
    expect(() =>
      ids([module("m", "# info m", "dependencies: base >= 1.3"), base]),
    ).toThrow(/needs base >= 1.3, but base 1.2.0 is loaded/);
    expect(() =>
      ids([module("m", "# info m", "dependencies: ? base = 1.0.0"), base]),
    ).toThrow(/needs \? base = 1.0.0, but base 1.2.0 is loaded/);
  });

  it("reads dependencies as a block as well as inline", () => {
    const parsed = parseModuleSource(
      module("m", "# info m", "dependencies:", "  base >= 1.0.0", "  ? extras"),
    );
    expect(parsed.info.dependencies).toEqual([
      {
        prefix: "required",
...
describe("cycles", () => {
  it("names the modules in a cycle", () => {
    const sources = [
      module("a", "# info a", "dependencies: b"),
      module("b", "# info b", "dependencies: a"),
    ];
  ...
  it("lets ~ break a cycle without weakening the requirement", () => {
    ...
    expect(() => ids([module("b", "# info b", "dependencies: ~ a")])).toThrow(
      /names a module that is not loaded: ~ a/,
    );
  });
});
...
describe("loadUniverse", () => {
  it("resolves references across module boundaries in either direction", () => {
    ...
    expect(
      loadUniverse([uses, stats]).entities.get("uses.ogre")!.stats,
    ).toEqual({ "stats.attack": { min: 4, max: 7 } });
  });
});
...
describe("loadUniverseWithDiagnostics", () => {
  it("skips disabled modules before parsing the strict load set, while reporting their pack status", () => {
    ...
    expect([...loadUniverse([base, extra]).items.keys()]).toEqual([
      "base.rope",
    ]);
  ...
  it("prunes references into absent optional modules instead of disabling the referring module", () => {
    ...
    expect(
      result.registry.items
        .get("base.charm")!
        .tags.map((tag) => (tag.kind === "stat-bonus" ? tag.statId : tag.kind)),
    ).toEqual(["base.guile"]);
    expect(result.registry.recipes.has("base.charm")).toBe(false);
    expect(result.registry.locations.get("base.camp")!.entities).toEqual([
      { entity: "base.chest" },
    ]);
  ...
  it("prunes what a passive carries, who carries it, and the position that placed it", () => {
    ...
    const { registry, loadedModules, diagnostics } =
      loadUniverseWithDiagnostics([base, addon]);

    expect({ loadedModules, diagnostics }).toEqual({
      loadedModules: ["base"],
      diagnostics: [],
    });
    expect(registry.entities.get("base.urchin")!.passives).toEqual([
      "base.spined",
    ]);
    ...
    expect(
      spined.tags.map((tag) =>
        tag.kind === "stat-bonus" ? tag.statId : tag.kind,
      ),
    ).toEqual(["base.guile"]);
    ...
    expect(spined.whenHit).toEqual([
      { kind: "give", item: "base.charm", amount: { min: 1, max: 1 } },
    ]);
  });
  ...
  it("takes an action-slug member with the action, at every site that prunes one, so a use: naming it cannot load clean", () => {
    ...
    const walker = (name: string, use: string): ModuleSource =>
      module(
        name,
        `# info ${name}`,
        "dependencies: base",
    ...
    for (const [name, address] of [
      ...
      const { registry, loadedModules, diagnostics } =
        loadUniverseWithDiagnostics([base, ghost, walker(name, address)]);
  ...
  it("reads the survivors off the whole universe, so pruning one object does not take an identically named object of another kind with it", () => {
    ...
    const walker = (use: string): ModuleSource =>
      module(
        "walk",
        "# info walk",
        "dependencies: base",
    ...
    const survivor = loadUniverseWithDiagnostics([
      base,
      ghost,
      walker("item.base.dresser.search-drawer"),
    ]);
    ...
    expect(survivor.registry.items.get("base.dresser")!.actions).toHaveLength(
      1,
    );
    expect(survivor.registry.namespace.declaredKeys("action-slug")).toEqual([
      "item.base.dresser.search-drawer",
    ]);
    ...
    const stranded = loadUniverseWithDiagnostics([
      base,
      ghost,
      walker("entity.base.dresser.search-drawer"),
    ]);
  ...
  it("disables only the module whose source does not parse", () => {
    const result = loadUniverseWithDiagnostics([
      module("base", "# info base", "# item rope"),
      { name: "broken", text: "# item" },
    ]);
    ...
    expect(formatModuleDiagnostic(result.diagnostics[0])).toContain(
      "broken:1:1 [broken] parse: # item requires an id",
    );
  });
  ...
  it("does not diagnose a switched-off source that does not parse, so switching one off is an exit from its problems", () => {
    const result = loadUniverseWithDiagnostics([
      module("base", "# info base", "# item rope"),
      { name: "broken", text: "# item", enabled: false },
    ]);
  ...
  it("disables a module that fails resolution, then disables dependents against the recomputed active set", () => {
    ...
    expect(
      result.diagnostics.map((d) => [d.moduleId, d.stage, d.message]),
    ).toEqual([
      [
        "bad",
        "resolve",
        '# entity bad.gull action "peck" give: names an unknown item: missing',
      ],
      [
        "child",
        "order",
        "# info child dependencies: names a module that is not loaded: bad",
      ],
    ]);
  });
...
describe("the parsed modules a load hands back", () => {
  it("are in the order they were applied, not the order the sources arrived in", () => {
    ...
    expect(result.parsed.map((each) => each.info.id)).toEqual([
      "alpha",
      "zebra",
    ]);
    expect(result.parsed.map((each) => each.info.id)).toEqual(
  ...
  it("carry the sections each module declared, which is what a serializer needs off them", () => {
    const result = loadUniverseWithDiagnostics([
      module("m", "# info m", "# item rock", "# variable pace", "value: 3"),
    ]);
    expect(result.parsed[0].sections.map((section) => section.kind)).toEqual([
      "item",
      "variable",
    ]);
  });
...
describe("a module declares the language it is written in (c5)", () => {
  ...
  it("defaults to en, which is where humanizeEn supplies an unauthored title", () => {
    expect(
      parseModuleSource(module("island", "# info island")).info.language,
    ).toBe("en");
  ...
  it("records a generated title as an entry only for a module writing English", () => {
    ...
    expect(es.locales.addressable.has("island.entity.giant-rat.title")).toBe(
      true,
    );
  });
  ...
  it("takes a tag it has never heard of, because nothing but the humanizeEn gate reads one", () => {
    expect(
      parseModuleSource(module("island", "# info island", "language: pt-br"))
        .info.language,
    ).toBe("pt-br");
  });
});
...
describe("a generated action label is an English entry too (c5)", () => {
  ...
  it("records the generated label only for a module writing English", () => {
    expect(
      isla("en").locales.base.get("isla.action.abrir-puerta.abrir-puerta"),
    ).toEqual({ text: "Abrir Puerta", language: "en", generated: true });
    expect(
      isla("es").locales.base.has("isla.action.abrir-puerta.abrir-puerta"),
    ).toBe(false);
    expect(isla("es").locales.base.has("isla.entity.puerta.abrir-puerta")).toBe(
      false,
    );
  });

  it("leaves the label itself alone, because it is what a player is shown", () => {
    expect(isla("es").actions.get("isla.abrir-puerta")?.label).toBe(
      "Abrir Puerta",
    );
  });

  it("records an authored label whatever language it is in", () => {
    const titled = loadUniverse([
      module(
        "isla",
        "# info isla",
        "version: 1.0.0",
