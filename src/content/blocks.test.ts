...
import { parseModule, parserFor, schemaFor } from "./sections";
import { SECTION_KINDS } from "./sections";
import { parseSaveSection } from "./sections/save";
...
function sectionSources(text: string): SectionSource[] {
  const render = (
    lines: readonly { text: string; children: readonly unknown[] }[],
    depth: number,
  ): string[] =>
    lines.flatMap((line) => [
      "  ".repeat(depth) + line.text,
      ...render(
        line.children as { text: string; children: readonly unknown[] }[],
        depth + 1,
      ),
    ]);
  return splitSections(text).map((section) => ({
    kind: section.kind,
    lines: [
      `# ${section.kind}${section.id === undefined ? "" : ` ${section.id}`}`,
      ...render(section.body, 0),
    ],
  }));
}
...
const shippedSections = (): SectionSource[] =>
  readdirSync("content")
    .filter(
      (name) =>
        name.endsWith(".dsl") &&
        name.replace(/\.dsl$/, "") !== LOCAL_CHANGES_MODULE_ID,
    )
...
const UNPROBED_BY_CONTENT: readonly {
  kind: string;
  lines?: string[];
  refusesABody?: string[];
}[] = [
  ...
  {
    kind: "remove",
    refusesABody: ["# remove entity.mirror", "reason: retired"],
  },
];

const parsed = (lines: readonly string[]) =>
  JSON.stringify(parseModule(lines.join("\n")));
...
describe("an indented block under a line whose reader never asked for one", () => {
  const written = UNPROBED_BY_CONTENT.filter(
    (entry) => entry.lines !== undefined,
  ).map((entry) => ({ kind: entry.kind, lines: entry.lines! }));
  ...
  it("is refused, never discarded, on every line the corpus writes", () => {
    const discarded = [
      ...new Set(
        probes
          .filter((probe) => probe.dropped)
          .map((probe) => `${probe.kind}: ${probe.line}`),
      ),
    ];
  ...
  it("probes every kind the loader can parse", () => {
    ...
    const excused = new Set(
      UNPROBED_BY_CONTENT.filter(
        (entry) => entry.refusesABody !== undefined,
      ).map((entry) => entry.kind),
    );
    expect(
      [...SECTION_KINDS]
        .sort()
        .filter((kind) => !probed.has(kind) && !excused.has(kind)),
    ).toEqual([]);
  });

  it("has a body refused outright by each kind it excuses for having no probeable line", () => {
    for (const entry of UNPROBED_BY_CONTENT.filter(
      (each) => each.refusesABody !== undefined,
    )) {
      expect(
        () => parsed(entry.refusesABody!),
        `# ${entry.kind} was excused as taking no body`,
      ).toThrow();
    }
  });
  ...
  it("is refused on a line whose block one reader looked at and no reader took", () => {
    expect(() =>
      parsed([
        "# faction birds",
        "# entity gull",
        "faction: birds aggressive",
        "  " + INTRUDER,
      ]),
    ).toThrow(/"faction: birds aggressive" takes no indented block/);
    expect(() =>
      parsed(["# faction birds", "# entity gull", "faction: birds aggressive"]),
    ).not.toThrow();
  });
  ...
  it("is refused by a section parser called directly, not only through the module table", () => {
    const saveSection = (...lines: string[]) =>
      splitSections(lines.join("\n"))[0];
    expect(() =>
      parseSaveSection(
        saveSection("# save start", '{"version":11}', "  " + INTRUDER),
      ),
    ).toThrow(/takes no indented block/);
    expect(() =>
      parseSaveSection(saveSection("# save start", '{"version":11}')),
    ).not.toThrow();
    ...
    expect(() =>
      parseAnySection(
        saveSection("# location bay", "starting", "  " + INTRUDER),
        location,
      ),
    ).toThrow(/takes no indented block/);
    expect(() =>
      parseAnySection(saveSection("# location bay", "starting"), location),
    ).not.toThrow();
  });
  ...
  it("is demanded by every kind the loader can parse, whoever wrote its parser", () => {
    ...
    const reached = new Set([
      ...probes.map((probe) => probe.kind),
      ...UNPROBED_BY_CONTENT.map((entry) => entry.kind),
    ]);
    expect([...reached].filter((kind) => !checked.includes(kind))).toEqual([]);
    expect(
      checked.filter((kind) => !answersForItsBlocks(parserFor(kind)!)),
    ).toEqual([]);
  });
  ...
  it("found lines to probe, in every kind that ships one", () => {
    expect(
      probes
        .filter((probe) => probe.unreadable !== undefined)
        .map((probe) => `${probe.line} — ${probe.unreadable}`),
    ).toEqual([]);
    expect(probes.length).toBeGreaterThan(100);
    for (const entry of written)
      expect(
        probes.filter((probe) => probe.kind === entry.kind).length,
        `# ${entry.kind} was written here to be probed`,
      ).toBeGreaterThan(0);
  });
});
