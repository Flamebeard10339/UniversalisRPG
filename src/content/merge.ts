...
const respecified = (value: unknown): boolean =>
  value !== undefined && !(Array.isArray(value) && value.length === 0);

export const overlay = (into: Fields, from: Fields): Fields => {
  const merged = { ...into };
  for (const [key, value] of Object.entries(from))
    if (respecified(value)) merged[key] = value;
  return merged;
};
...
function identifies(pattern: unknown, candidate: unknown): boolean {
  ...
  return Object.entries(pattern).every(([key, value]) =>
    identifies(value, (candidate as Fields)[key]),
  );
}
...
function applyEdits(held: unknown, edits: FieldEdits): unknown[] {
  ...
  for (const { op, values: operands } of edits.ops) {
    for (const operand of operands) {
      if (op === "-")
        values = values.filter((value) => !identifies(operand, value));
      else if (!values.some((value) => identifies(operand, value)))
        values.push(operand);
    }
  }
  return values;
  ...
  for (const [key, value] of Object.entries(from)) {
    if (key === entries)
      merged[key] = mergeEntries(
        (into[key] as Labelled[]) ?? [],
        value as Labelled[],
      );
    else if (isListField(schema, key) && isFieldEdits(value))
      merged[key] = applyEdits(into[key], value);
