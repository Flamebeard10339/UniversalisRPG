...
describe("local-changes module text helpers", () => {
  ...
  it("upserts one section by kind and id while preserving the other staged sections", () => {
    ...
    text = upsertLocalSection(
      text,
      ["base"],
      "# entity sprite\ntitle: Sprite\n",
    ).text;
    const edited = upsertLocalSection(
      text,
      ["base"],
      "# item gem\ntitle: Ruby\n",
    );
    ...
    expect(localSectionHeadings(edited.text)).toEqual([
      "# item gem",
      "# entity sprite",
    ]);
  ...
  it("keeps every header line the file owns, in place, across an edit and a delete", () => {
    ...
    const staged = upsertLocalSection(
      foreign,
      ["base"],
      "# item ruby\ntitle: Ruby\n",
    );
    expect(header(staged.text)).toEqual([
      "# info local-changes",
      "version: 3.2.1",
      "pack: shared",
      "dependencies:",
    ...
    expect(localSectionHeadings(staged.text)).toEqual([
      "# item gem",
      "# item ruby",
    ]);
