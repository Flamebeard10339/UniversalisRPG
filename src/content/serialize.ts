...
export interface SerializeModuleOptions {
  info: Pick<ModuleInfo, "id"> &
    Partial<Pick<ModuleInfo, "version" | "dependencies" | "pack" | "language">>;
...
function infoLines(info: SerializeModuleOptions["info"]): Lines {
  ...
  if (info.language !== undefined && info.language !== DEFAULT_LANGUAGE)
    lines.push(`language: ${info.language}`);
  if (info.dependencies && info.dependencies.length > 0)
    lines.push(
      "dependencies:",
      ...indentLines(info.dependencies.map((each) => dependency.print(each))),
    );
  return lines;
}
...
const tableOf = (
  ...
  mapOf(registry, registryMapOf(kind)!) as unknown as ReadonlyMap<
    string,
    object
  >;
...
function printedSections(
  ...
  for (const kind of SECTION_KINDS) {
    ...
    if (owner.ids === "global") {
      ...
      for (const id of options.globals ?? []) {
        for (const global of GLOBAL_SECTION_KINDS) {
          ...
          if (value !== undefined)
            printed.push({ id, section: sectionOf(global, value) });
        }
      }
      continue;
    }
    ...
    for (const [id, value] of tableOf(registry, kind))
      if (inModule(moduleId, id))
        printed.push({ id, section: sectionOf(kind, value) });
  }
  return printed;
}
...
function sectionText(
  ...
  const namespace =
    sectionFor(section.kind)!.ids === "global" ? null : moduleId;
  return printSectionOf(section, {
    moduleId,
    id,
    authored: (field) => {
      const entry = registry.locales.base.get(
        localeKey(namespace, section.kind, id, field),
      );
...
function serializeRegistryModule(
  registry: Registry,
  options: SerializeModuleOptions,
): string {
  const sections = printedSections(registry, options).map((each) =>
    sectionText(registry, options.info.id, each),
  );
  return (
    [infoLines(options.info).join("\n"), ...sections].join("\n\n").trimEnd() +
    "\n"
  );
}
...
function compare(
  loaded: Registry,
  printed: string,
  checked: UniverseLoadResult,
): RoundTrip {
  if (checked.diagnostics.length > 0)
    return { printed, diagnostics: checked.diagnostics, differences: [] };
  return {
    printed,
    diagnostics: [],
...
export function roundTripModule(
  loaded: Registry,
  options: SerializeModuleOptions,
...
export function roundTripUniverse(
  loaded: Registry,
  modules: readonly ParsedModule[],
  reload: (printed: readonly ModuleSource[]) => UniverseLoadResult,
): UniverseRoundTrip {
...
export const canSerialize = (module: ParsedModule): boolean =>
