...
const fightResults = (source: string): ActionResult[] =>
  loadModule(source).entities.get("giant-rat")!.actions[0].results;

describe("the four selectors", () => {
  it("reads authored odds, and refuses the three that are not odds", () => {
    expect(fightResults(rat("1 in 5:", "  give: 1 bones"))).toEqual([
      {
        kind: "chance",
        numerator: 1,
        denominator: 5,
        results: [{ kind: "give", item: "bones", amount: { min: 1, max: 1 } }],
      },
    ]);
    expect(load(rat("0 in 5: give: 1 bones"))).toThrow(/0 in 5 never happens/);
    expect(load(rat("6 in 5: give: 1 bones"))).toThrow(
      /6 in 5 is more than certain/,
    );
    expect(load(rat("1 in 0: give: 1 bones"))).toThrow(
      /1 in 0 is not a chance/,
    );
  });
  ...
  it("reads a body inline or as a block, and refuses both at once or neither", () => {
    ...
    expect(load(rat("1 in 5: give: 1 bones", "  give: 1 coins"))).toThrow(
      /written inline and as a block/,
    );
  ...
  it("nests a wrapper inside a wrapper, which is what layering a drop is", () => {
    const results = fightResults(
      rat("1 in 5:", "  1 in 3:", "    give: 1 gem"),
    );
    expect(results[0]).toMatchObject({
      kind: "chance",
      numerator: 1,
      denominator: 5,
      results: [{ kind: "chance", numerator: 1, denominator: 3 }],
    });
  });
    ...
    expect(results).toEqual([
      {
        ...
        rows: [
          { weight: 12, results: [] },
          {
            weight: 5,
            results: [
              { kind: "give", item: "coins", amount: { min: 20, max: 20 } },
            ],
          },
          {
            weight: 1,
            results: [
              { kind: "give", item: "gem", amount: { min: 1, max: 1 } },
              {
                kind: "say",
                text: "It glitters.",
                key: "entity.giant-rat.say.0",
              },
            ],
          },
        ],
  ...
  it("refuses a one of: that can select nothing, and a vs read as a weight", () => {
    ...
    expect(
      load(rat("one of:", "  0x: give: 1 bones", "  1x: give: 1 coins")),
    ).toThrow(/row "0x" can never be selected/);
    expect(load(rat("one of:", "  1x:"))).toThrow(/has an empty body/);
    expect(
      load(
        "# stat luck\n" +
          rat("one of:", "  luck vs 60: give: 1 bones", "  1x: give: 1 coins"),
      ),
    ).toThrow(/a vs contest is an independent chance, not a weight/);
  });
  ...
  it("reads a contest from stats or literals on either side", () => {
    ...
    expect(
      fightResults("# stat luck\n" + rat("luck vs 60: give: 1 gem"))[0],
    ).toMatchObject({ kind: "contest", left: "luck", right: 60 });
  });

  it("reads a gate, and a row spells its own gate in its selector", () => {
    expect(
      fightResults(
        "# flag lit\n" + rat("if lit and has bones:", "  give: 1 gem"),
      )[0],
    ).toMatchObject({ kind: "gate" });
    const rows = fightResults(
      "# flag lit\n" +
        rat("one of:", "  12x: nothing", "  1x if lit: give: 1 gem"),
    );
    expect(rows[0]).toMatchObject({
      kind: "one-of",
      rows: [{ weight: 12 }, { weight: 1, requires: { kind: "reference" } }],
    });
    expect(
      (rows[0] as { rows: { requires?: unknown }[] }).rows[0].requires,
    ).toBeUndefined();
  });

  it("refuses a range on either side of the odds, not just the numerator", () => {
    const odds =
      /is odds, not a quantity, so it takes one number rather than a range/;
  ...
  it("leaves prose that merely begins with a selector word alone", () => {
    ...
    const prose = [
      ...
      "  one of us is lying:",
      "  3-4 in every ten make it back.",
      "  1 in 5 never do.",
    ];
    const dialogue = loadModule(
      [
        "# entity miki",
        "# dialogue chat",
        "owner = miki",
    ...
    expect(
      dialogue.dialogues.get("chat")!.nodes[0].steps.map((step) => step.kind),
    ).toEqual(prose.map(() => "say"));
  });
  ...
  it("reads a complete vs contest as one, even where a narrator meant prose", () => {
    expect(
      load(
        ...
        "owner = miki",
        "node a:",
        "  luck vs fighting: the old question.",
      ),
    ).toThrow(/unrecognized action result/);
    ...
    expect(
      load(
        ...
        "owner = miki",
        "node a:",
        "  luck vs fighting is the old question.",
      ),
    ).not.toThrow();
  });
});
...
describe("# droptable", () => {
  ...
  const universe = (...lines: string[]) =>
    loadModule([...ITEMS, ...TABLES, ...lines].join("\n"));

  it("is a named result list reached by roll:", () => {
    const registry = universe(
      "# entity giant-rat",
      "fight:",
      "  roll: rat-common",
      "  1 in 128:",
    ...
    expect(registry.dropTables.get("rat-common")!.results[0]).toEqual({
      kind: "give",
      item: "bones",
      amount: { min: 1, max: 1 },
    });
    expect(registry.entities.get("giant-rat")!.actions[0].results[0]).toEqual({
      kind: "roll",
      table: "rat-common",
    });
  });
  ...
  it("refuses an empty table, an unknown roll, and a table that reaches itself", () => {
    ...
    expect(load(...ITEMS, "# entity g", "p:", "  roll: nowhere")).toThrow(
      /roll: names an unknown droptable: nowhere/,
    );
    expect(
      load("# droptable a", "roll: b", "# droptable b", "roll: a"),
    ).toThrow(/# droptable a rolls itself: a -> b -> a/);
    expect(
      load(...ITEMS, "# droptable self", "give: 1 bones", "roll: self"),
    ).toThrow(/rolls itself/);
  });

  it("takes a removal, and goes with a member it can no longer name", () => {
    const removed = loadModule(
      [...ITEMS, ...TABLES, "# remove droptable.rare-drop"].join("\n"),
    );
  ...
  it("prints an action whose only result is a wrapper as a block, not as one line", () => {
    ...
    const trip = roundTripModule(
      registry,
      { info: { id: "pack", version: [1, 0, 0] } },
      (text) => loadUniverseWithDiagnostics([{ name: "again", text }]),
    );
...
describe("produced quantities carry the range they were written as", () => {
  ...
  it("takes a range on a recipe out: and burnt:, which is the fletching case", () => {
    ...
    expect(recipe.out).toEqual([
      { item: "arrow", amount: { min: 5, max: 10 } },
    ]);
  ...
  it("refuses a range where the number is consumed, and says which it is", () => {
    expect(load("# item x", "# entity g", "p:", "  take: 5-10 x")).toThrow(
      /consumed, so it takes one number rather than a range/,
    );
    expect(
      load("# item x", "# item y", "# recipe r", "in: 2-4 x", "out: 1 y"),
    ).toThrow(/consumed, so it takes one number rather than a range/);
    expect(load("# flag c", "# entity g", "p:", "  add: c 1-3")).toThrow(
      /add: takes one signed count rather than a range/,
    );
  });
  ...
  it("refuses a range at every threshold, and says which it is", () => {
    ...
    expect(
      load(
        ...
        "# entity g",
        "p:",
        "  requires: has 5-10 potion",
        "  say: hi",
      ),
    ).toThrow(threshold);
    expect(
      load("# flag c", "# entity g", "p:", "  requires: c >= 1-3", "  say: hi"),
    ).toThrow(threshold);
    expect(
      load(
        "# item x",
    ...
    expect(
      load("# item x", "# entity g", "p:", "  attempts: 3-5", "  give: 1 x"),
    ).toThrow(threshold);
  });
  ...
  it("refuses a range in a selector, where odds are not a quantity at all", () => {
    const odds =
      /is odds, not a quantity, so it takes one number rather than a range/;
    expect(
      load("# item x", "# entity g", "p:", "  1-2 in 5: give: 1 x"),
    ).toThrow(odds);
    expect(
      load(
        "# item x",
  ...
  it("inverts the zero rule: a floor of zero is the point, a ceiling of zero is not", () => {
    expect(
      loadModule(
        "# item straw\n# entity gull\npeck:\n  give: 0-3 straw",
      ).entities.get("gull")!.actions[0].results[0],
    ).toEqual({ kind: "give", item: "straw", amount: { min: 0, max: 3 } });
    expect(
      load("# item straw", "# entity gull", "peck:", "  give: 0 straw"),
    ).toThrow(/a count of 0 does nothing: straw/);
    expect(
      load("# item straw", "# entity gull", "peck:", "  give: 0-0 straw"),
    ).toThrow(/a count of 0 does nothing: straw/);
    expect(
      load(
        "# stat x",
