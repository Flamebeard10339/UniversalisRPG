...
describe("# cluster-jewel", () => {
  ...
  it("rejects a shape that does not exist, listing the ones that do", () => {
    expect(
      load("# cluster-jewel a", "shape: hexagram", "open-connections: e"),
    ).toThrow(/point, spindle, ring, wheel, double-ring/);
  });
  ...
  it("rejects a position outside the shape's range", () => {
    ...
    ).toThrow(/# cluster-jewel a.*position 4 is outside spindle's 1-3 range/s);
  });
  ...
  it("rejects a position filled twice", () => {
    expect(
      load(
        ...
        "shape: spindle",
        "open-connections: e",
        "passives: 1 hale, 1 mending",
      ),
    ).toThrow(/# cluster-jewel a.*position 1 is filled twice/s);
  });
  ...
  it("rejects a passive that does not resolve, as an ordinary load-time reference error", () => {
    ...
    ).toThrow(/# cluster-jewel a passives: names an unknown passive: nope/);
  });
});
...
describe("open-connections", () => {
  it("accepts all five non-root edges at once", () => {
    const registry = loadModule(
      [
        ...
        "# cluster-jewel a",
        "shape: point",
        "open-connections: ne, nw, e, sw, se",
      ].join("\n"),
    );
    expect(registry.clusterJewels.get("a")!.openConnections).toEqual([
      "ne",
      "nw",
      "e",
      "sw",
  ...
  it("rejects the west edge, which the root occupies", () => {
    expect(
      load("# cluster-jewel a", "shape: point", "open-connections: w"),
    ).toThrow(/# cluster-jewel a.*west edge/s);
  });

  it("rejects the same edge named twice", () => {
    expect(
      load("# cluster-jewel a", "shape: point", "open-connections: e, e"),
    ).toThrow(/# cluster-jewel a.*names e more than once/s);
  });

  it("rejects zero open connections, the structural non-termination guarantee", () => {
    expect(load("# cluster-jewel a", "shape: point")).toThrow(
      /# cluster-jewel a.*at least one edge/s,
    );
  });

  it("rejects a direction that is not one of the six", () => {
    expect(
      load("# cluster-jewel a", "shape: point", "open-connections: north"),
    ).toThrow(/# cluster-jewel a.*unknown direction: north/s);
  });
});
