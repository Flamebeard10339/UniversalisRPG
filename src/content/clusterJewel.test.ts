...
const load = (...lines: string[]) => () => loadModule([PASSIVES, ...lines].join('\n'));
...
describe('# cluster-jewel', () => {
  ...
  it('reads passives one pair to a line, the way # entity reads stats: as a block', () => {
    ...
    expect(registry.clusterJewels.get('blood-frenzy')!.positions).toEqual({ 1: 'hale', 10: 'mending' });
  });
...
describe('clusterJewelProblem', () => {
  ...
  it('finds nothing wrong with a jewel inside its shape\'s range', () => {
    expect(clusterJewelProblem({ id: 'a', title: 'A', shape: 'spindle', openConnections: ['e'], positions: { 1: 'hale' }, modSlots: 2 }, shape)).toBeUndefined();
  });

  it('reports a position outside the shape', () => {
    expect(clusterJewelProblem({ id: 'a', title: 'A', shape: 'spindle', openConnections: ['e'], positions: { 9: 'hale' }, modSlots: 2 }, shape)).toMatch(/position 9 is outside spindle's 1-3 range/);
  });
});
