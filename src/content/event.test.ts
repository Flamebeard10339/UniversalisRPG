...
const withEvent = (body: string): string =>
  `${PRELUDE}\n# event moment\n${body}\n`;
...
// line it is added.
const wellFormed = (trigger: EventTrigger): string =>
  withEvent(
    `trigger: ${trigger}${watchesAPool(trigger) ? "\nresource: health" : ""}`,
  );
...
describe("the closed set lives in trigger:", () => {
  it("accepts every name the table declares and nothing else", () => {
    for (const trigger of TRIGGER_NAMES)
      expect(() => loadModule(wellFormed(trigger)), trigger).not.toThrow();
    for (const absent of [
      "succeeded",
      "failed",
      "escaped",
      ...
      expect(
        () => loadModule(withEvent(`trigger: ${absent}\nresource: health`)),
        absent,
      ).toThrow("event trigger must be one of");
    }
  });
  ...
  it("names the triggers that do exist when it refuses one, so the list is readable off the error", () => {
    try {
      ...
      for (const trigger of TRIGGER_NAMES)
        expect(message, trigger).toContain(trigger);
    }
  });

  it("is the whole vocabulary: eight names, each declaring one arity", () => {
    ...
    expect(
      Object.values(EVENT_TRIGGERS).every(
        (arity) => arity === "pool" || arity === "none",
      ),
    ).toBe(true);
  });
});
...
describe("a trigger's arity is resource:, in either direction", () => {
  it("refuses a declaration that disagrees with what its trigger takes", () => {
    for (const trigger of TRIGGER_NAMES) {
      ...
      const [loads, refused] = watchesAPool(trigger)
        ? [watching, bare]
        : [bare, watching];
      expect(() => loadModule(loads), `${trigger} well-formed`).not.toThrow();
      expect(() => loadModule(refused), `${trigger} ill-formed`).toThrow(
        /watches a pool, so it needs a resource:|watches no pool, so it takes no resource:/,
      );
    }
  });

  it("names the arity it violated rather than only the field", () => {
    expect(() => loadModule(withEvent("trigger: on empty"))).toThrow(
      "trigger: on empty watches a pool, so it needs a resource: naming which one",
    );
    expect(() =>
      loadModule(withEvent("trigger: damage-taken\nresource: health")),
    ).toThrow(
      "trigger: damage-taken watches no pool, so it takes no resource:",
    );
  });
});
...
describe("a grant names a declared event and never an engine moment", () => {
  it("refuses every trigger name written where an event name belongs", () => {
    for (const trigger of TRIGGER_NAMES) {
      ...
      expect(() => loadModule(source), trigger).toThrow(
  ...
  it("resolves a declared event exactly as a handler label does", () => {
    const registry = loadModule(
      `${PRELUDE}gain 4 experience on moment\n\n# event moment\ntrigger: damage-dealt\n\n# entity rat\non moment:\n  say: bitten\n`,
    );
    expect(registry.skills.get("melee")!.grants).toEqual([
      { coefficient: 4, amount: false, event: "moment" },
    ]);
...
describe("a grant into a module that is absent prunes like every other reference", () => {
  it("drops the grant and keeps the skill, so the module still loads", () => {
    ...
    expect(registry.skills.get("m.melee")!.grants).toEqual([
      { coefficient: 2, amount: false, event: "m.moment" },
    ]);
  });
  ...
  it("still refuses a name that no absent module could have supplied", () => {
    expect(() =>
      loadModule(
        OPTIONAL.replace(
          "gain 4 experience on absent.gone",
          "gain 4 experience on typo",
        ),
      ),
    ).toThrow("names an unknown event");
  });
});
