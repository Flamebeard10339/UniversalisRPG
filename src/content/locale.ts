...
export const isEngineKey = (key: string): key is EngineKey =>
  ENGINE_KEY_SET.has(key);

export { textFieldsOf } from "./sections";
...
const KEY =
  /^(?<key>[a-z0-9][a-z0-9-]*(?:\.[a-z0-9][a-z0-9-]*)*):[ \t]?(?<value>.*)$/;

export const parseLocaleSection = sectionParser(
  (section: RawSection): LocaleSection => {
    if (!section.id)
      throw new DslError(
        "# locale requires a language, as in `# locale en`",
        section.span,
      );
    ...
    for (const line of section.body) {
      if (hasBlock(line))
        throw new DslError(
          `# locale ${section.id}: a translation is one line`,
          line.span,
        );
      const groups = KEY.exec(line.text)?.groups;
      if (!groups)
        throw new DslError(
          `# locale ${section.id}: expected \`<key>: <text>\`, got ${JSON.stringify(line.text)}`,
          line.span,
        );
      if (seen.has(groups.key))
        throw new DslError(
          `# locale ${section.id}: ${groups.key} is translated more than once`,
          line.span,
        );
      seen.add(groups.key);
...
export function localeKey(
  ...
  return [namespace, kind, localId(namespace, id), field]
...
export const dialogueSayField = (node: string, index: number): string =>
  `${node}.say.${index}`;
export const dialogueLineField = (node: string, index: number): string =>
  `${node}.line.${index}`;
export const dialogueChoiceField = (node: string, index: number): string =>
  `${node}.choice.${index}`;
...
const FIELD_NAMES: ReadonlySet<string> = new Set(
  SECTIONS.flatMap((each) => each.text),
);
...
// digit in it.
export function actionSlugProblem(
  slug: string,
  label: string,
  taken: ReadonlySet<string>,
): string | undefined {
  if (!/^[a-z0-9][a-z0-9-]*$/.test(slug))
    return `action ${JSON.stringify(label)} has no address: it keys as ${JSON.stringify(slug)}, so give it a label with a letter or a digit in it`;
  if (FIELD_NAMES.has(slug))
    return `action ${JSON.stringify(label)} keys as ${slug}, which is already a field of the object that owns it`;
  if (taken.has(slug))
...
export const parametersOf = (pattern: string): string[] =>
  [...pattern.matchAll(PARAM)].map((match) => match[1]);
...
export function unsuppliedParameters(
  locales: Locales,
  key: string,
  value: string,
): string[] {
...
export function addLocaleSection(
  locales: Locales,
  module: string | null,
  ...
  for (const { key, value } of section.entries) {
    table.set(key, value);
    if (section.id === DEFAULT_LOCALE && !locales.english.has(key))
      locales.english.set(key, value);
  }
  locales.declared.set(section.id, table);
}

// What a module declared, for printing it back out.
export function moduleLocaleSections(
  locales: Locales,
  module: string | null,
): LocaleDeclaration[] {
...
export function missingTranslations(
  ...
  for (const key of [...ENGINE_KEYS, ...locales.addressable]) {
    if (declared?.has(key) || locales.base.get(key)?.language === language)
      continue;
    missing.push(key);
  }
  return missing;
