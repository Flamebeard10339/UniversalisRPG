...
export const registryDiffMaps = (): readonly string[] =>
  CONTENT_SECTION_MAPS.map(([, map]) => map);
...
export function registryDiff(before: Registry, after: Registry): string[] {
  ...
  for (const line of left)
    if (!right.has(line)) lines.push(`  locales: missing ${line}`);
  const held = new Set(left);
  for (const line of right)
    if (!held.has(line)) lines.push(`  locales: added ${line}`);
  for (const name of registryDiffMaps()) {
    ...
    for (const key of [...left.keys()].sort()) {
      ...
      else if (!sameValue(left.get(key), right.get(key)))
        lines.push(`  ${name}: changed ${key}`);
    }
    for (const key of [...right.keys()].sort())
      if (!left.has(key)) lines.push(`  ${name}: added ${key}`);
  }
  return lines;
}
