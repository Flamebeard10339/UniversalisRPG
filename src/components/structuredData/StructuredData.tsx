...
const PrimitiveEditor = ({ accessibleLabel, schema, value, onChange }: { accessibleLabel?: string; schema: Exclude<StructuredSchema, { kind: 'object' | 'array' | 'union' | 'inferred' }>; value: StructuredValue | undefined; onChange: (value: StructuredValue) => void }) => {
...
const EditorNode = ({ hiddenKeys = [], label, onChange, optional, schema: schemaRef = { kind: 'inferred' }, t, value }: EditorProps) => {
  ...
  if (schema.kind === 'object') {
    const record = value && typeof value === 'object' && !Array.isArray(value) ? value : {};
    const fieldNames = Array.from(new Set([...Object.keys(schema.fields), ...Object.keys(record)])).filter((key) => !hiddenKeys.includes(key));
    return <section className="grid gap-2">{label && <div className="flex items-center justify-between"><span className="text-xs font-semibold uppercase tracking-wide text-slate-500">{labelText(label, t)}</span>{optional && <button className="text-xs text-rose-300" onClick={() => onChange(undefined)} type="button">{t('structured.remove')}</button>}</div>}{fieldNames.map((key) => { const field = schema.fields[key] ?? { schema: { kind: 'inferred' } as StructuredSchema, optional: true }; return <div className="min-w-0" key={key}><EditorNode label={field.label ?? key} onChange={(next) => { const updated = { ...record }; if (next === undefined) delete updated[key]; else updated[key] = next; onChange(updated); }} optional={field.optional || !(key in record)} schema={field.schema} t={t} value={record[key]} /></div>; })}{schema.allowAdditional && <div className="flex gap-2"><input className={inputClass} onChange={(event) => setNewField(event.target.value)} placeholder={t('structured.fieldName')} value={newField} /><button className="rounded border border-slate-600 px-2 py-1 text-xs" disabled={!newField.trim() || newField in record} onClick={() => { onChange({ ...record, [newField.trim()]: '' }); setNewField(''); }} type="button">{t('structured.addField')}</button></div>}</section>;
  }

  return <div className="flex min-w-0 items-center gap-2">{label && <span className="min-w-32 text-xs text-slate-400">{labelText(label, t)}</span>}<PrimitiveEditor accessibleLabel={label ? labelText(label, t) : undefined} onChange={onChange} schema={schema as Extract<StructuredSchema, { kind: 'string' | 'number' | 'boolean' | 'enum' }>} value={value} />{optional && <button className="text-xs text-rose-300" onClick={() => onChange(undefined)} type="button">{t('structured.remove')}</button>}</div>;
};
