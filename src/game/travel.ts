...
// Reuses the same per-node edge computation the map/pathfinding use — this
// both labels an autogenerated "Travel to X (Ns)" button and (via
// getActionDurationMs in adversarial.ts) is what actually governs how long
// that button takes to resolve when clicked directly, so the two can never
// drift apart. A travel action's own `durationSeconds` is never consulted
// for this — see `validation.travelDurationUnused`.
