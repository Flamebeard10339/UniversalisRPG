...
export const HOOK_LABELS: readonly string[] = Object.values(HOOK_FIELDS).map(
  (field) => field.keyword!,
);

const CARRIED =
  "is carried by a character rather than by a verb — write it on the `# entity` or `# item` that carries it, because an action is the verb and is shared by everyone who performs it";

const AIMED =
  "was never implemented — there is one `on hit:` per carrier, read off the character carrying it, and a result that reaches the other party says so itself, as in `drain: 3 health from them`";
const STRUCK =
  "was never implemented — the moment a swing lands on the carrier is `when hit:`, and it is read off the carrier like the other one";
...
const RETIRED_HOOK_LABELS: Readonly<Record<string, string>> =
  Object.fromEntries([
    ...
    ...[...HOOK_LABELS, ...RETIRED_MOMENTS].flatMap((moment) =>
      RECIPIENTS.map((recipient) => [`${moment} ${recipient}`, AIMED]),
    ),
  ]);
