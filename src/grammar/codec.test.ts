...
import { collectionFailures, exportedCodecs, isCodec, reachableCodecs, roundTripFailures } from './codec';
...
describe('the law itself', () => {
  ...
  it('reports a parser whose print does not return what was parsed', () => {
    expect(roundTripFailures('brittle', brittle)).toEqual(['brittle: "007" printed back as "7"']);
  });

  it('reports a parser carrying no examples rather than passing it', () => {
    expect(roundTripFailures('empty', { ...brittle, examples: [] })).toEqual(['empty carries no examples']);
  });

  it('reports a parser that refuses its own example', () => {
    const refusing: Parser<number> = { ...brittle, examples: ['nope'] };
    expect(roundTripFailures('refusing', refusing)[0]).toContain('unexpected content');
  });

  it('is not satisfied by a value that only parses', () => {
    expect(isCodec({ parse: (cursor: Cursor) => cursor.rest() })).toBe(false);
    expect(isCodec({ parse: () => 1, print: () => '', examples: [] })).toBe(true);
  });
  ...
  it('follows a list parser to its element, so a wrapper cannot hide one', () => {
    ...
    expect([...reachableCodecs([['wrapper', wrapper]]).values()]).toEqual(['wrapper', 'wrapper.element']);
  });
});
