...
export type BonusAmount =
  { percent: false; amount: Range } | { percent: true; amount: number };
...
export type Counter =
  { kind: "resource"; id: string } | { kind: "stack"; id: string };
...
export type TagClause =
  ...
  | ({ kind: "stat-bonus"; statId: string; per?: Counter } & BonusAmount)
...
const CLAUSE_HOLDS: {
  ...
  "stat-bonus": (clause) =>
    ...
    (clause.percent === true
      ? typeof clause.amount === "number" && Number.isFinite(clause.amount)
      : clause.percent === false && isRange(clause.amount)),
  duration: (clause) =>
    typeof clause.seconds === "number" && Number.isFinite(clause.seconds),
};
...
export function isTagClause(value: unknown): boolean {
  ...
  const holds = CLAUSE_HOLDS[clause.kind as TagClause["kind"]] as
    ((clause: Record<string, unknown>) => boolean) | undefined;
...
const STAT_BONUS = new RegExp(
  `^${AMOUNT}[ \t]+(?<stat>${NAME})(?:[ \t]+per[ \t]+(?:stack[ \t]+of[ \t]+(?<stack>${NAME})|(?<per>${NAME})))?$`,
);
...
function parseAmount(
  ...
  if (groups.percent === "%") {
    if (groups.hi !== undefined)
      throw new DslError(
        `a percent stat bonus cannot be a range: ${JSON.stringify(raw)}`,
        span,
      );
  ...
  if (groups.hi === undefined)
    return { percent: false, amount: { min: sign * lo, max: sign * lo } };

  const hi = Number(groups.hi);
  if (hi < lo)
    throw new DslError(
      `a stat bonus range must ascend in magnitude, got ${JSON.stringify(raw)}`,
      span,
    );
  return {
    percent: false,
...
function parseClause(raw: string, span: Span): TagClause {
  ...
  if (duration && (duration.minutes || duration.seconds)) {
    return {
      kind: "duration",
      seconds:
        Number(duration.minutes ?? 0) * SECONDS_PER_MINUTE +
  ...
  if (bonus) {
    ...
    if (bonus.stack !== undefined)
      return { ...clause, per: { kind: "stack", id: bonus.stack } };
    return bonus.per === undefined
      ? clause
      : { ...clause, per: { kind: "resource", id: bonus.per } };
  }
...
function printAmount(value: BonusAmount): string {
  if (value.percent)
    return `${value.amount < 0 ? "-" : "+"}${Math.abs(value.amount)}%`;
...
const printCounter = (value: Counter): string =>
  value.kind === "stack" ? `stack of ${value.id}` : value.id;
...
export const bonusAmount: Parser<BonusAmount> = {
  parse(cursor) {
    ...
    if (!groups)
      throw new DslError(
        `expected a bonus like +1 or +1%, got ${JSON.stringify(raw)}`,
        span,
      );
