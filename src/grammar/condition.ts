...
export const isEngineRoot = (path: readonly string[]): boolean =>
  ENGINE_ROOTS.includes(path[0]);
...
export const visitedNode = (
  path: readonly string[],
): readonly string[] | null =>
  path.length > 1 && path[path.length - 1] === VISITS
    ? path.slice(0, -1)
    : null;
...
function parseNot(cursor: Cursor): Condition {
  if (cursor.take(/not[ \t]+/) !== null)
    return { kind: "not", condition: parseNot(cursor) };
  return parsePrimary(cursor);
}

function parseAnd(cursor: Cursor): Condition {
  const conditions = [parseNot(cursor)];
  while (cursor.take(/[ \t]+and[ \t]+/) !== null)
    conditions.push(parseNot(cursor));
...
function parseOr(cursor: Cursor): Condition {
  ...
  while (cursor.take(/[ \t]+or[ \t]+/) !== null)
    conditions.push(parseAnd(cursor));
...
export const printReference = (value: Reference): string =>
  value.path.join(".");
  ...
  switch (value.kind) {
    ...
    case "has":
      return value.count === 1
        ? `has ${value.item}`
        : `has ${number.print(value.count)} ${value.item}`;
    case "not":
