...
const ACTION_KEYWORD_TAGS: ReadonlySet<string> = new Set<string>(
  TAGGED_ACTION_KINDS,
);
...
const RETIRED_ACTION_TAGS: Readonly<Record<string, string>> = {
  ...
  retaliates:
    'tag "retaliates" was retired — one two-sided action is brought by whoever swings, so an entity retaliates with the actions it `uses:` and nothing marks a block as the owner\'s own',
};

export const actionKind = (action: Action): ActionKind =>
...
const conditionValue: ActionValue = (cursor) =>
  cursor.done ? undefined : condition.parse(cursor);
const contestValue =
...
const resultsValue: ActionValue = (cursor, line, label) => {
  if (cursor.done)
    return hasBlock(line) ? results.parseBlock(takeBlock(line)) : undefined;
  const inline = results.parse(cursor);
  if (hasBlock(line))
    throw new DslError(
      actionProblem(
        label,
        "a result group is written inline and as a block; give it one",
      ),
      line.span,
    );
  return inline;
...
function named<T>(
  ...
  try {
    ...
    throw new DslError(
      actionProblem(label, `${written}: ${error.message}`),
      line.span,
    );
  }
}
...
const seconds: ActionValue = (cursor, line, label) =>
  named("time", label, line, () => decimal.parse(cursor));
...
const positiveCount =
  ...
  (cursor, line, label) => {
    ...
    if (raw === null || Number(raw) <= 0)
      throw new DslError(
        actionProblem(label, `${written} requires a positive integer`),
        line.span,
      );
    named(written, label, line, () =>
      refuseRange(
        cursor,
...
const RETIRED_ACTION_FIELDS: readonly { label: RegExp; message: string }[] = [
  {
    label: /speed:[ \t]*/,
    message:
      "speed: was retired — write rate: for attempts per minute, either a number or the stat holding one",
  },
  {
    label: /evasion:[ \t]*/,
    message:
      "evasion: was retired — it is the right half of one line, `accuracy: my accuracy vs their evasion`",
  },
  {
    label: /ability:[ \t]*/,
    message:
      "ability: was retired — it is the left half of one line, `damage: my attack vs their defense`",
  },
  {
    label: /dr:[ \t]*/,
    message:
      "dr: was retired — it is the right half of one line, `damage: my attack vs their defense`",
  },
  {
    label: /target:[ \t]*/,
    message:
      "target: was retired — write `depletes: their <pool>` for the pool a landed hit reduces",
  },
  {
    label: /escape after[ \t]+/,
    message:
      "escape after was retired — write `attempts: N`, which bounds the action at N of the performer's attempts",
  },
  {
    label: /on escape:[ \t]*/,
    message:
      "on escape: was retired — write `on unfinished:`, which runs when `attempts:` ran out before the action completed",
  },
...
function parseActionField(
  ...
  for (const retired of RETIRED_ACTION_FIELDS) {
    if (cursor.take(retired.label) !== null)
      throw new DslError(actionProblem(label, retired.message), line.span);
  }
  for (const field of ACTION_FIELDS) {
    if (cursor.take(field.label) === null) continue;
    if (held[field.name] !== undefined)
      throw new DslError(
        actionProblem(label, `${field.written} is defined more than once`),
        line.span,
      );
    if (appends && !APPENDABLE.has(field.written))
      throw new DslError(
        actionProblem(
          label,
          `${field.written} holds one value, so + has nothing to add to — write it bare to replace what this block overlays`,
        ),
        line.span,
      );
...
const isSided = (value: number | Sided | undefined): value is Sided =>
...
export function sidedFields(
  action: Action,
): { written: string; value: Sided }[] {
  ...
  if (action.depletes)
    found.push({ written: "depletes", value: action.depletes });
  return found;
}
...
export const sideOf = (field: Sided, self: string, other: string): string =>
  field.side === "their" ? other : self;
...
export const isTwoSided = (action: Action): boolean =>
  sidedFields(action).some((field) => field.value.side !== undefined);
...
export function actionTableProblem(action: Action): string | undefined {
  const cadence = [
    action.time !== undefined && "time:",
    action.rate !== undefined && "rate:",
  ].filter((written): written is string => written !== false);
  if (cadence.length > 1)
    return "time: and rate: are the same axis written two ways; give one";

  const kind = actionKind(action);
  if (kind === "instant" && cadence.length > 0)
    return `an instant action takes no ${cadence[0]}`;
  ...
  if (kind === "continuous" && cadence.length === 0)
    return "a continuous action needs a time: or rate: to set its pace";

  const written = action.time !== undefined ? "time:" : "rate:";
  const value =
    action.time ?? (typeof action.rate === "number" ? action.rate : undefined);
  if (value !== undefined && !(value > 0))
    return `${written} must be positive — something that takes no time carries no cadence at all`;

  if (!isTwoSided(action)) return undefined;
  const unmarked = sidedFields(action).find(
    (field) => field.value.side === undefined,
  );
  if (unmarked)
    return `${unmarked.written}: ${unmarked.value.id} names no side — write ${SIDES.map((side) => `${side} ${unmarked.value.id}`).join(" or ")}, because this action already names one`;
  return undefined;
}
...
export function assembledActionProblem(action: Action): string | undefined {
  ...
  if (isTwoSided(action) && !action.depletes)
    return "a side-naming action with nothing to deplete is not a contest — write `depletes: their <pool>`";
  return undefined;
}
...
export const actionProblem = (label: string, problem: string): string =>
  `action ${JSON.stringify(label)}: ${problem}`;
...
function checkTags(
  action: Omit<Action, "label">,
  label: string,
  span: RawLine["span"] | undefined,
): void {
  for (const tag of action.tags ?? []) {
    if (tag.kind === "duration")
      throw new DslError(
        actionProblem(
          label,
          "a duration clause paces nothing on an action — write `time: <seconds>` or `rate: <per minute>`",
        ),
        span,
      );
    ...
    throw new DslError(
      actionProblem(
        label,
        retired ??
          `unknown tag ${JSON.stringify(tag.value)} — an action's bare tags are ${[...ACTION_KEYWORD_TAGS].join(", ")}`,
      ),
      span,
    );
  }
...
function resolveKind(
  ...
  const tagged = TAGGED_ACTION_KINDS.filter((kind) =>
    (action.tags ?? []).some(
      (tag) => tag.kind === "keyword" && tag.value === kind,
    ),
  );
  if (tagged.length > 1)
    throw new DslError(
      actionProblem(label, `cannot be both ${tagged.join(" and ")}`),
      span,
    );
...
function printResultBlock(
  ...
  lines.push(
    `${label}:`,
    ...indentLines(values.flatMap(resultLines), childSpaces),
  );
}

const printSided = (value: Sided): string =>
  value.side === undefined ? value.id : `${value.side} ${value.id}`;

const printContest = (value: Contest): string =>
  value.right === undefined
    ? printSided(value.left)
    : `${printSided(value.left)} vs ${printSided(value.right)}`;
...
export function actionLines(action: Action): string[] {
  ...
  if (!modifiers && action.results.length === 1 && !spansLines(action.results))
    return [`${action.label}: ${resultList.print(action.results)}`];
  ...
  const at = (name: keyof Action): string =>
    appended.has(name) ? "  +" : "  ";
  const lines: string[] = [`${action.label}:`];
  if (action.requires)
    lines.push(
      `${at("requires")}requires: ${condition.print(action.requires)}`,
    );
  if (action.hiddenIf)
    lines.push(
      `${at("hiddenIf")}hidden if: ${condition.print(action.hiddenIf)}`,
    );
  if (action.kind !== undefined && action.kind !== "duration")
  ...
  const tags = (action.tags ?? []).filter(
    (each) => each.kind !== "keyword" || !lifted.has(each.value),
  );
  if (tags.length > 0)
    lines.push(`  ${tags.map((each) => tagClause.print(each)).join(", ")}`);
  if (action.time !== undefined) lines.push(`  time: ${action.time}`);
  if (action.rate !== undefined)
    lines.push(
      `  rate: ${typeof action.rate === "number" ? action.rate : printSided(action.rate)}`,
    );
  if (action.accuracy)
    lines.push(`  accuracy: ${printContest(action.accuracy)}`);
  ...
  if (action.attempts !== undefined)
    lines.push(`  attempts: ${action.attempts}`);
