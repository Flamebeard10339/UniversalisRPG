...
// both are subjects.
export function reachableCodecs(
  roots: Iterable<readonly [string, unknown]>,
): Map<Parser<unknown>, string> {
...
export function roundTripFailures(
  ...
  return parser.examples.flatMap((example) => {
    ...
    try {
      ...
      return [
        `${name}: ${JSON.stringify(example)} threw ${error instanceof Error ? error.message : String(error)}`,
      ];
    }
    return printed === example
      ? []
      : [
          `${name}: ${JSON.stringify(example)} printed back as ${JSON.stringify(printed)}`,
        ];
  });
}
...
export const collectionFailures = (
  codecs: Map<Parser<unknown>, string>,
): string[] =>
  [...codecs].flatMap(([parser, name]) => roundTripFailures(name, parser));
