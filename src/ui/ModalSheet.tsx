      ...
      <div className="mx-auto w-full max-w-2xl rounded-2xl border border-border bg-surface-raised p-4">
        ...
        {option.values ? (
          <div className="flex max-h-[60vh] flex-col gap-2 overflow-y-auto">
            {option.values.map((value) => (
              <button
                ...
                className="min-h-[48px] w-full rounded-xl border border-border bg-panel px-4 py-2 text-left active:bg-accent-strong active:text-accent-text"
              >
                {value}
              </button>
            ))}
          ...
          >
            ...
            />
            <button type="submit" className="min-h-[48px] min-w-[48px] rounded-xl bg-accent text-lg text-accent-text">
              ▸
            </button>
          </form>
        )}
