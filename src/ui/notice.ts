...
function climbed<Row extends { id: Answer }>(before: readonly Row[], after: readonly Row[], under: string, amount: (row: Row) => number, said: (row: Row) => Localized): Notice[] {
  ...
  return after.flatMap((row) => {
    ...
    return risen > 0 ? [{ key: `${under}:${row.id}`, count: risen, words: said(row) }] : [];
  });
}
...
const RAISED_BY: readonly Watcher[] = [
  ...
  (before, after, words) =>
    crossings(before.xp, after.xp).flatMap((id) => {
      ...
      return row === undefined ? [] : [{ key: `level:${id}`, count: 0, words: words('levelled', { skill: row.title, level: row.level }) }];
    }),
  (before, after) => {
    ...
    return after.journal.flatMap((entry) => {
      ...
      return was === undefined || was === standingOf(entry) ? [] : [{ key: `quest:${entry.quest}`, count: 0, words: standingLine(entry) ?? entry.title }];
    });
  },
];
